				...
				struct msdos_dir_entry **de)
{
 ...
 int find_volume(struct inode *inode,char * new_name,unsigned int len)
{
	loff_t cpos;
	int ret = 0;
	...
	if (inode->i_ino != MSDOS_ROOT_INO){
		printk("volume must be in ROOT DIR\n");
		return ret;	
	}
...
get_new:
	...
	{
		printk("found the old volume :%s\n",de->name);
		//printk("2 bh->b_size =%d, %ld\n",bh->b_size,bh->b_state);	
		memset(de->name,0,MSDOS_NAME);
		memcpy(de->name,(void *)(new_name+7),len);
		printk("replace with new volume: %s \n",de->name);
...
static inline unsigned char nls_tolower_fat(struct nls_table *t, unsigned char c)
{
...
static inline int nls_strnicmp_fat(struct nls_table *t, const unsigned char *s1,
		const unsigned char *s2, int len)
{
	while (len--) {
		if (nls_tolower_fat(t, *s1++) != nls_tolower_fat(t, *s2++))
			return 1;
	}
				 ...
				 const unsigned char *b, int b_len)
{
	...
	if (sbi->options.name_check != 's')
		return !nls_strnicmp_fat(sbi->nls_io, a, b, a_len);
	else
