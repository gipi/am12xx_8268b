...
ntfschar  Q[3] = { const_cpu_to_le16('$'), const_cpu_to_le16('Q'),
		const_cpu_to_le16('\0') };
ntfschar  R[3] = { const_cpu_to_le16('$'), const_cpu_to_le16('R'),
		const_cpu_to_le16('\0') };
...
int ntfs_check_empty_dir(ntfs_inode *ni)
{
	...
	err = ntfs_attr_lookup(AT_INDEX_ROOT, I30, 4,//找到目的type
	...
	/* Non-empty directory? */
	if ((a->data.resident.value_length != sizeof(INDEX_ROOT) + sizeof(INDEX_ENTRY_HEADER))){
 ...
 int ntfs_check_unlinkable_dir(ntfs_inode *ni, FILE_NAME_ATTR *fn)
{
	...
	link_count= le16_to_cpu(m->link_count);/*hard link count*/
	 ...
	 */
	if ((link_count == 1) || 
	    (link_count == 2 && fn->file_name_type == FILE_NAME_DOS)) {/*DOS name and WIN32 name exsit together, so hard link count is actual one*/
		ret = -1;
		ntfs_error(vi->i_sb,"Non-empty directory without hard links,link_count = %d",link_count);
		//goto no_hardlink;
	}
 ...
 * the dcache aliasing issues.
 如果是不区分大小写的匹配，@res返回文件名的小端unicode码；
 如果找到的文件名是DOS(8.3)格式，则@res返回DOS格式的文件名。
 ntfs_lookup函数将利用该DOS文件名去寻找节点的长文件名。
 *
		...
		const int uname_len, ntfs_name **res)
{
	...
	DEBUG0_ON(!S_ISDIR(VFS_I(dir_ni)->i_mode));
	DEBUG0_ON(NInoAttr(dir_ni));
		...
		u8 *name, void *dirent, filldir_t filldir)
{
	...
	if (MREF_LE(ie->data.dir.indexed_file) < FILE_first_user &&
		...
		ntfs_trace("Skipping system file.");
		return 0;
	}
	...
	if (ia_page)
		unlock_page(ia_page);
	ntfs_trace("Calling filldir for %s with len %i, fpos 0x%llx, inode "
...
static int ntfs_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	fpos = filp->f_pos;
	ntfs_trace("Entering for inode 0x%lx, fpos 0x%llx.",
			vdir->i_ino, fpos);
	rc = err = 0;
	...
	if (!fpos) {
		...
		rc = filldir(dirent, ".", 1, fpos, vdir->i_ino, DT_DIR);	//添加'.'
		if (rc)
			goto done;
		fpos++;
	}
	...
	if (fpos >= vol->mft_record_size)	 //目录节点的第一个属性就为INDEX ROOT?
	...
	ir_pos = (s64)fpos;
	/* Find the index root attribute in the mft record. 0x90*/
	err = ntfs_attr_lookup(AT_INDEX_ROOT, I30, 4, CASE_SENSITIVE, 0, NULL,
			0, ctx);
	if (unlikely(err)) {
	...
	memcpy(ir, (u8*)ctx->attr +		//拷贝INDEX ROOT值到ir
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		...
		rc = ntfs_filldir(vol, fpos, ndir, NULL, ie, name, dirent, 	//将INDEX ROOT中的INDEXT ENTRY信息一次添加
				filldir);
		if (rc) {
			kfree(ir);
			goto abort;
	...
	if (!NInoIndexAllocPresent(ndir))	//判断是否有INDEX ALLOCATION 0XA0属性
		goto EOD;
...
skip_index_root:
	...
	ia_pos = (s64)fpos - vol->mft_record_size; //==0
	ia_mapping = vdir->i_mapping;
	ntfs_debug("Inode 0x%lx, getting index bitmap.", vdir->i_ino);
	bmp_vi = ntfs_attr_iget(vdir, AT_BITMAP, I30, 4);	//根据属性类型，生成新的fake inode
	if (IS_ERR(bmp_vi)) {
		ntfs_error(sb, "Failed to get bitmap attribute.");
		err = PTR_ERR(bmp_vi);
		goto err_out;
	}
	bmp_mapping = bmp_vi->i_mapping;	//得到bitmap的address mapping
...
get_next_bmp_page:
	ntfs_trace("Reading bitmap with page index 0x%llx, bit ofs 0x%llx",
			(unsigned long long)bmp_pos >> (3 + PAGE_CACHE_SHIFT),
			(unsigned long long)bmp_pos &
			(unsigned long long)((PAGE_CACHE_SIZE * 8) - 1));
	bmp_page = ntfs_map_page(bmp_mapping,	//得到bitmap值
...
find_next_index_buffer:
	...
	ntfs_trace("Handling index buffer 0x%llx.",
	...
	/* Get the current index buffer. */ //得到0xa0 INDEX ALLOCATION的值
	 ...
	 */
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		ntfs_trace("In index allocation, offset 0x%llx.",
...
abort:
	...
	if (!rc)
		...
		ntfs_trace("filldir returned %i, fpos 0x%llx, returning 0.",
				rc, fpos);
#endif
	filp->f_pos = fpos;
	return 0;
