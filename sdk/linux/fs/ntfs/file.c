 ...
 * @cached_page:	store any allocated but unused page here 存储分配的但是没使用的page
 ...
 * these are simply marked dirty).
 将@ni描述的属性的初始化大小扩展为@new_init_size bytes，
 涉及的操作有:将old initialized size和@new_init_size bytes之间的非稀疏空间清零，
 同时更新相关的page chce和磁盘数据。
 *
		...
		struct page **cached_page, struct pagevec *lru_pvec)
{
	...
	DEBUG0_ON(new_init_size > ni->allocated_size);
	...
	DEBUG0_ON(old_init_size != old_i_size);
	...
	DEBUG0_ON(old_i_size != (loff_t)attr_len);
	/*
...
do_non_resident_extend:
	...
	read_lock_irqsave(&ni->size_lock, flags);
	DEBUG0_ON(ni->initialized_size != new_init_size);
	read_unlock_irqrestore(&ni->size_lock, flags);
 ...
 *填充实际簇的洞，分配足够的buffer head，保证bh都映射了，
 将bh更新。
		...
		unsigned nr_pages, s64 pos, size_t bytes)
{
	...
	ni = NTFS_I(vi);
	vol = ni->vol;
	ntfs_trace("Entering for inode 0x%lx, attribute type 0x%x, start page "
...
rl_not_mapped_enoent:
		...
		ntfs_trace("Allocated cluster, lcn 0x%llx.",
 ...
 *将所有的bh 标记为脏，如果一个page中所有的bh都uptodate了，
 则设置该page也uptodate。
 
 * Setting the buffers dirty ensures that they get written out later when
 * ntfs_writepage() is invoked by the VM.
 * 将bh标记为脏的目的是当vm进程调用 ntfs_writepage()时，能将其写入磁盘
		...
		const unsigned nr_pages, s64 pos, size_t bytes)
{
	...
	ntfs_trace("Entering for inode 0x%lx, attribute type 0x%x, start page "
			...
			(long long)pos, bytes);
	if (NInoNonResident(ni))//对于非驻留属性
		return ntfs_commit_pages_after_non_resident_write(pages,
				nr_pages, pos, bytes);
	DEBUG0_ON(nr_pages > 1);
	/*
	...
	kattr = (u8*)a + le16_to_cpu(a->data.resident.value_offset);//属性的值
	kaddr = kmap_atomic(page, KM_USER0);	//建立临时映射
	/* Copy the received data from the page to the mft record. */
	memcpy(kattr + pos, kaddr + pos, bytes);//将page的内容拷贝到属性
	 ...
	 */
	if (!PageUptodate(page)) {//page没更新
		if (pos > 0)
			memcpy(kaddr, kattr, pos);
		if (end < attr_len)
			memcpy(kaddr + end, kattr + end, attr_len - end);//将属性中的内容拷贝到page
...
/**pos :?*/
#ifdef  NTFS_MODIFY	
 ssize_t ntfs_attr_write(ntfs_inode * ni, const ATTR_TYPE type,
		ntfschar *name, u32 name_len,loff_t pos, size_t count, const  unsigned char * buf)
{
	...
	read_unlock_irqrestore(&ni->size_lock, flags);
	ntfs_debug("pos = 0x%llx ,ni->initialized_size = 0x%llx",pos,ni->initialized_size);
	if (pos > ll) {
		...
		loff_t pos, loff_t *ppos, size_t count)
{
	 ...
	 * fails again.
	 文件大小被改变! 一共会尝试2次
	 */
	if (unlikely(NInoTruncateFailed(ni))) {//如果之前改变文件大小失败，再试试
		if(ni->mft_no == 0)
			ntfs_debug("down_write");
