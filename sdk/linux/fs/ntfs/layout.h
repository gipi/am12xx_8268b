 ...
 * been written to disk.  
 usa: 修正值(修正序列)，位于每个扇区末尾的2bytes
 usn: 该受保护的记录写入磁盘的次数；
 *The values 0 and -1 (ie. 0xffff) are not used. All
...
typedef struct {
	NTFS_RECORD_TYPE magic;	/* A four-byte magic identifying the record
				   type and/or status. */
	le16 usa_ofs;		/* Offset to the Update Sequence Array (usa) 被修正值相对于ntfs record起始的偏移
				   from the start of the ntfs record. */
	le16 usa_count;		/* Number of le16 sized entries in the usa修正个数=uas_count -1
...
其中属性名和属性值都没在该结构体中定义，
而是直接存在磁盘上，需要时通过offset来读取
 */
typedef struct {//标准属性头
/*Ofs*/
...
typedef le32 FILE_ATTR_FLAGS;	//文件类型
 ...
 */
		...
		/* 52*/	le32 security_id;	/* Security_id for the file. 通过$secure的$SII /$SDS获得
 ...
 * It is not allowed to have two attributes with all sorting keys equal.
 Attribute list 属性包含了文件的每一个属性的入口，
 但不包括其本身。list的分类准则:先按属性类型;
 再按属性名字(如果存在),最后按instance号；
 非驻留属性的extents紧接着初始的extent，它们按
 lowest_vcn排序且instance为0。
 两个不同的属性不能有相同的分类key。
 
 * - Further restrictions:
 *	- If not resident, the vcn to lcn mapping array has to fit inside the
 *	  base mft record.
 如果非驻留，vcn->lcn的对应序列必须位于主mft record中；
 
 *	- The attribute list attribute value has a maximum size of 256kb. This
 *	  is imposed by the Windows cache manager.
  Attribute list 属性最大为256KB；
  
 * - Attribute lists are only used when the attributes of mft record do not
 * fit inside the mft record despite all attributes (that can be made
 * non-resident) having been made non-resident. This can happen e.g. when:
当mft record容不下所有属性(尽管属性都被转为非驻留)时，
使用 Attribute lists。
	这使得以下情况变得乐观:
 *	- File has a large number of hard links (lots of file name
 *	  attributes present).
 文件有大量的硬链接(因此有许多文件名属性)
...
/*  7*/	u8 name_offset;		/* Byte offset to beginning of attribute name
				   ...
				   start even if unnamed). 属性名相对于属性描述的偏移*/
...
typedef u8 FILE_NAME_TYPE_FLAGS;//文件名类型
...
enum {
	 ...
	 */
	SMALL_INDEX = 0, /* The index is small enough to fit inside the index //小目录
			    root attribute and there is no index allocation
			    attribute present. */
	LARGE_INDEX = 1, /* The index is too large to fit in the index root	//大目录
	 ...
	 */
	LEAF_NODE  = 0, /* This is a leaf node, i.e. there are no more nodes		//叶节点也就是末节点
			   branching off it. */
	INDEX_NODE = 1, /* This node indexes other nodes, i.e. it is not a leaf	//非叶节点
			   node. */
 ...
 */
typedef struct {
/*0x10*/	le32 entries_offset;		/* Byte offset to first INDEX_ENTRY	//第一个索引项（目录项）相对于索引(目录)头的偏移
					   aligned to 8-byte boundary. */
/*0x14*/	le32 index_length;		/* Data size of the index in bytes,	//所有索引项的总尺寸
					   i.e. bytes used from allocated
					   size, aligned to 8-byte boundary. */
/*0x18*/	le32 allocated_size;		/* Byte size of this index (block),	//所有索引项的总分配尺寸
					   multiple of 8 bytes. */
	/* NOTE: For the index root attribute, the above two numbers are always
	   ...
	   belongs to. */
/*0x1c*/		INDEX_HEADER_FLAGS flags;	/* Bit field of INDEX_HEADER_FLAGS. */
	u8 reserved[3];			/* Reserved/align to 8-byte boundary. */
} __attribute__ ((__packed__)) INDEX_HEADER;

/*
 * Attribute: Index root (0x90).$Index_Root
 *
 * NOTE: Always resident.
 *
 * This is followed by a sequence of index entries (INDEX_ENTRY structures)
 * as described by the index header.紧接着 index entries 之后
 *
 ...
 * index block.
 当目录很小时，只有$Index_Root一个属性；
 当目录大小超出$Index_Root时，还有$Index_Allocation索引根分布，
 它包括了B+树的子节点。另外还有$bitmap属性，描述索引根中的vcn使用情况
 ...
 */
typedef struct {
/*0x00*/	ATTR_TYPE type;			/* Type of the indexed attribute. Is
					   $FILE_NAME for directories, zero
					   for view indexes. No other values
					   allowed. */
/*0x04*/	COLLATION_RULE collation_rule;	/* Collation rule used to sort the
					   index entries. If type is $FILE_NAME,
					   this must be COLLATION_FILE_NAME. */
/*0x08*/	le32 index_block_size;		/* Size of each index block in bytes (in
					   the index allocation attribute).在$Index_Allocatio中的索引块尺寸 */
/*0x0c*/	       u8 clusters_per_index_block;	/* Cluster size of each index block (in  //每个索引块占用的簇数
...
/* 16*/	leVCN index_block_vcn;	/* Virtual cluster number of the index block.该index block对应的VCN ,从0递增
...
/* sizeof() = 16 bytes */
} __attribute__ ((__packed__)) INDEX_ENTRY_HEADER;	//index entry的头
...
/*  0	INDEX_ENTRY_HEADER; -- Unfolded here as gcc dislikes unnamed structs. */
	union {
		struct { /* Only valid when INDEX_ENTRY_END is not set. */
			leMFT_REF indexed_file;	/* The mft reference of the file //文件名的mft reference
						   described by this index
						   entry. Used for directory
						   indexes. */
		} __attribute__ ((__packed__)) dir;
	...
	le16 length;		 /* Byte size of this index entry, multiple of	//整个记录的长度
				    8-bytes. */
	le16 key_length;	 /* Byte size of the key value, which is in the //如:文件名的长度
				    index entry. It follows field reserved. Not
				    multiple of 8-bytes. */
...
} __attribute__ ((__packed__)) INDEX_ENTRY;//文件或目录在其父目录中的存放记录
 ...
 * bitmap中的bit number *index block size /簇大小==Index allocation中的簇数
 */
typedef struct {
