...
void ntfs_inode_mark_dirty(ntfs_inode *ni)
{
	NInoSetDirty(ni);
	if (ni->nr_extents == -1)
		NInoSetDirty(ni->ext.base_ntfs_ino);
}
...
{
	if (NInoDirty(ni))
		ntfs_error("Releasing dirty inode %lld!\n", 
			       (long long)ni->mft_no);
	if (NInoAttrList(ni) && ni->attr_list)
		free(ni->attr_list);
	//free(ni->mrec);
	unmap_mft_record(ni);//释放MFT记录
	free(ni);
	return;
}
#endif
 ...
 * 对于0xa0属性，用ntfs_index_iget代替ntfs_attr_iget；
		...
		ntfschar *name, u32 name_len)
{
	...
	DEBUG0_ON(type == AT_INDEX_ALLOCATION); 
...
/** 当type为0xa0时，ntfs_attr_iget的替换函数
...
void ntfs_destroy_big_inode(struct inode *inode)
{
	...
	//ntfs_debug("Entering. %ld",ni->mft_no);
	DEBUG0_ON(ni->page);
...
static int ntfs_read_locked_attr_inode(struct inode *base_vi, struct inode *vi)
{
	...
	a = ctx->attr;
	if (a->flags & (ATTR_COMPRESSION_MASK | ATTR_IS_SPARSE)) {
		if (a->flags & ATTR_COMPRESSION_MASK) { //压缩
			NInoSetCompressed(ni);
			if ((ni->type != AT_DATA) || (ni->type == AT_DATA && //只能为数据属性
					ni->name_len)) {
			...
			if (vol->cluster_size > 4096) {	//簇大小不能大于4096
				ntfs_error(vi->i_sb, "Found compressed "
	...
	if (!a->non_resident) {	//驻留属性
		...
		if (NInoMstProtected(ni)) { //不能被修正值修正
			ntfs_error(vi->i_sb, "Found mst protected attribute "
		...
		vi->i_size = ni->initialized_size = le32_to_cpu( //初始化数据大小
				a->data.resident.value_length);
		ni->allocated_size = le32_to_cpu(a->length) -
				le16_to_cpu(a->data.resident.value_offset);
		if (vi->i_size > ni->allocated_size) {
 ...
 * or attribute inode as well as that @vi is a base inode.
 vi不能是目录、索引、属性节点,且vi必须是主节点
 *
 * Returns 0 on success or -errno on error.成功:返回0
 *
 ...
 * without holding ->i_alloc_sem.
 调用时hold i_mutex锁，但是i_alloc_sem被写 操作持有时例外。
...
int ntfs_truncate(struct inode *vi)
{
	...
	DEBUG0_ON(NInoAttr(ni));
	DEBUG0_ON(S_ISDIR(vi->i_mode));//不能是目录节点
	DEBUG0_ON(NInoMstProtected(ni));
	DEBUG0_ON(ni->nr_extents < 0);//vi必须是主节点
retry_truncate:
	/*
	...
	if (NInoNonResident(ni))
		...
		new_alloc_size = (new_size + 7) & ~7;	//新分配大小
	/* The current allocated size is the old allocated size. */
	read_lock_irqsave(&ni->size_lock, flags);
	old_alloc_size = ni->allocated_size;		//老分配大小
	...
	if (new_size < vol->mft_record_size &&
			!ntfs_resident_attr_value_resize(m, a, new_size)) {//改变mft record中属性记录的大小
...
int ntfs_write_inode(struct inode *vi, int sync)
{
	 ...
	 */
	if (modified) { //修改了标准属性后，将mft record 标记为脏
		flush_dcache_mft_record_page(ctx->ntfs_ino);
	...
	if (NInoDirty(ni))
		err = write_mft_record(ni, m, sync);	//将主节点的mft record写回到磁盘
	...
	if (ni->nr_extents > 0) {
		...
		for (i = 0; i < ni->nr_extents; i++) {
			...
			if (NInoDirty(tni)) {
				...
				ret = write_mft_record(tni, tm, sync); //将所有子节点也写回到磁盘
 ...
 *首先检查extent inode是否已经附加到base inode，如果是直接返回
 ...
 *如果否，重新分配一个新的ntfs inode，并用给定的mref初始化
 mref指定了inode号/mft记录号，以及序列号
 ...
 *然后给mft记录分配一个buffer，从主inode的vol中读取mft record,
 将该buffer赋给ntfs inode。
...
ntfs_inode *ntfs_extent_inode_open(ntfs_inode *base_ni, const MFT_REF mref)
{
	...
	if (!base_ni) {
		//errno = EINVAL;
		ntfs_error("%s", __FUNCTION__);
		return NULL;
	}
	//将mref代表的从记录添加到base_ni代表的主记录
	...
	if (base_ni->nr_extents > 0) {
		extent_nis = base_ni->extent_ntfs_inos;//主节点的从节点列表
		for (i = 0; i < base_ni->nr_extents; i++) {
			...
			if (seq_no && seq_no != le16_to_cpu(
					m->sequence_number)) {
				//errno = EIO;
				ntfs_error("Found stale extent mft "
					"reference mft=%lld",
					(long long)ni->mft_no);
				goto out;
			}
	...
	ni->mft_no = mft_no;
	ni->nr_extents = -1;
	ni->base_ntfs_ino = base_ni;//从节点的主节点
	...
	if (!(base_ni->nr_extents & 3)) {
		...
		if (base_ni->nr_extents) {
			memcpy(extent_nis, base_ni->extent_ntfs_inos,
					i - 4 * sizeof(ntfs_inode *));
			free(base_ni->extent_ntfs_inos);
		}
		base_ni->extent_ntfs_inos = extent_nis;
	}
...
int ntfs_attr_record_move_away(ntfs_attr_search_ctx *ctx, int extra)
{
	...
	if (!ctx || !ctx->attr || !ctx->ntfs_ino || extra < 0) {	
		ntfs_error(vi->i_sb,"ctx=%p ctx->attr=%p extra=%d", 
	...
	if (!NInoAttrList(base_ni)) {		
		ntfs_error(vi->i_sb,"Inode %llu has no attrlist", 
				(unsigned long long)base_ni->mft_no);
		return -EINVAL;
	}

	if ((ntfs_inode_attach_all_extents(ctx->ntfs_ino)) != 0) {
		ntfs_error(vi->i_sb,"Couldn't attach extents, inode=%llu", 
				(unsigned long long)base_ni->mft_no);
		return -1;
	}
	...
	for (i = 0; i < base_ni->nr_extents; i++) {
		 ...
		 * do not return error.
		 */
		if ((ntfs_attr_record_move_to(ctx, ni)) != 0)
			return 0;
	}
	...
	if (IS_ERR(ni)) {
		err = PTR_ERR(ni);
		ntfs_error(vi->i_sb,"Couldn't allocate MFT record");
		return err;
	}
	unmap_mft_record(ni);
	if (ntfs_attr_record_move_to(ctx, ni)) {
		ntfs_error(vi->i_sb,"Couldn't move attribute to MFT record");
		return -1;
	}
	return 0;
}

/**释放inode节点中mft record的空间
 * ntfs_inode_free_space - free space in the MFT record of an inode
 * @ni:		ntfs inode in which MFT record needs more free space
 * @size:	amount of space needed to free	需释放的空间大小
 *
...
int ntfs_inode_free_space(ntfs_inode *ni, int size)
{
	...
	if (!ni || size < 0) {	
		ntfs_error(vi->i_sb,"ni=%p size=%d", ni, size);
		return -EINVAL;
	}
	 ...
	 * record, so position search context on the first attribute after them.
	 0x10 0x20属性一定在主mft record中，因此跳过两者
	 */
	if ((err = ntfs_attr_position(AT_FILE_NAME, ctx)) != 0) //cannot find the attribute
		goto put_err_out;
...
retry:						
			if ((err = ntfs_attr_position(AT_UNUSED, ctx)) != 0) //cannot find the attribute
				goto put_err_out;
		}
		...
		if ((err = ntfs_attr_record_move_away(ctx, 0)) != 0) {
			ntfs_error(vi->i_sb,"Failed to move out attribute #2");
			break;
		}
		freed += record_size;
...
int ntfs_inode_add_attrlist(ntfs_inode *ni)
{
	...
	if (!ni) {		
		ntfs_error(vi->i_sb,"Invalid argument");
		return -EINVAL;
	}
	...
	if (NInoAttrList(ni) || ni->nr_extents) {		
		ntfs_error(vi->i_sb,"Inode already has attribute list");
		return -EEXIST;
	}

	/* Form attribute list. 新建属性列表*/
	...
	while ((err = ntfs_attr_lookup(AT_UNUSED, NULL, 0, 0, 0, NULL, 0, ctx)) != 0) {
		
		int ale_size;
		
		if (ctx->attr->type == AT_ATTRIBUTE_LIST) {
			err = -EIO;
			ntfs_error(vi->i_sb,"Attribute list already present");
			goto put_err_out;
		}
		/*该属性对应的attribute list entry 大小*/
		ale_size = (sizeof(ATTR_LIST_ENTRY) + sizeof(ntfschar) *	
					ctx->attr->name_length + 7) & ~7;	
		al_len += ale_size;
		
		aln = krealloc(al, al_len,GFP_KERNEL);
		if (!aln) {
			err = -ENOMEM;
			ntfs_error(vi->i_sb,"Failed to realloc %d bytes", al_len);
			goto put_err_out;
		}
		...
		if (ctx->attr->non_resident)
			ale->lowest_vcn = ctx->attr->data.non_resident.lowest_vcn;
		else
			ale->lowest_vcn = 0;
	...
	/*if (errno != ENOENT) {
		err = errno;
		ntfs_error("%s: Attribute lookup failed, inode %lld",
	...
	NInoSetAttrList(ni);		//该节点包含attrlist
	NInoSetDirty(ni);		//该节点需要写回到磁盘
	//NInoAttrListSetDirty(ni);	//该attrlist需写回到mft record 和磁盘
	...
	if (le32_to_cpu(mrec->bytes_allocated) -
		...
		if ((err = ntfs_inode_free_space(ni,
				offsetof(ATTR_RECORD, data.resident.resident_end))) != 0) {
			/* Failed to free space. */			
			ntfs_error(vi->i_sb,"Failed to free space for attrlist");
			goto rollback;
		}
	}
	...
	if (ntfs_resident_attr_record_add(ni,
		...
		//err = errno;
		ntfs_error(vi->i_sb,"Couldn't add $ATTRIBUTE_LIST to MFT");
		goto rollback;
	}
	...
	if (!na) {
		err = errno;
		ntfs_error("Failed to open just added $ATTRIBUTE_LIST");
		goto remove_attrlist_record;
	}
	if (ntfs_attr_truncate(na, al_len)) {
		err = errno;
		ntfs_error("Failed to resize just added $ATTRIBUTE_LIST");
		ntfs_attr_close(na);
		goto remove_attrlist_record;;
	}
#endif
	if((ntfs_attr_truncate(ni,AT_ATTRIBUTE_LIST, AT_UNNAMED, 0,al_len)) != 0)//by yangli
	{
...
//	ntfs_attr_close(na);
	...
	if ((ntfs_attr_lookup(AT_ATTRIBUTE_LIST, NULL, 0,
				CASE_SENSITIVE, 0, NULL, 0, ctx)) == 0) {//找到0x20属性
		if (ntfs_attr_record_rm(ctx))
			ntfs_error(vi->i_sb,"Rollback failed to remove attrlist");
	} else
		ntfs_error(vi->i_sb,"Rollback failed to find attrlist");
	...
	while ((u8*)ale < al + al_len) {
		if (MREF_LE(ale->mft_reference) != ni->mft_no) {
			if ((ntfs_attr_lookup(ale->type, ale->name,
						...
						NULL, 0, ctx))==0) {	//找到0x20属性
				if ((ntfs_attr_record_move_to(ctx, ni)) !=0)
					ntfs_error(vi->i_sb,"Rollback failed to "
							"move attribute");
			} else
				ntfs_error(vi->i_sb,"Rollback failed to find attr");
			ntfs_attr_reinit_search_ctx(ctx);
		}
...
int ntfs_inode_attach_all_extents(ntfs_inode *ni)
{
	...
	ntfs_error(vi->i_sb,"Entering for inode 0x%llx.", (long long) ni->mft_no);

	/* Inode haven't got attribute list, thus nothing to attach. */
	if (!NInoAttrList(ni))
		return 0;

	if (!ni->attr_list) {//? 属性列表是主节点用来记录次节点属性de ?
		ntfs_error(vi->i_sb,"Corrupt in-memory struct.");
		return -EINVAL;
	}
	...
	while ((u8*)ale < ni->attr_list + ni->attr_list_size) {
		if (ni->mft_no != MREF_LE(ale->mft_reference) &&
			...
			mrec = map_extent_mft_record(ni, ale->mft_reference,&extent_ni);
			if (IS_ERR(mrec)) {
				//err = PTR_ERR(mrec);
					ntfs_error(vi->i_sb,"Couldn't attach extent inode.");
					return -1;
			}
			unmap_mft_record(ni);
			prev_attached = MREF_LE(ale->mft_reference);
