...
static bool parse_ntfs_boot_sector(ntfs_volume *vol, const NTFS_BOOT_SECTOR *b)
{
	...
	vol->sector_size = le16_to_cpu(b->bpb.bytes_per_sector);//ÉÈÇø´óÐ¡
	...
	sectors_per_cluster_bits = ffs(b->bpb.sectors_per_cluster) - 1; //Ã¿´ØµÄÉÈÇøÊý
	ntfs_debug("sectors_per_cluster_bits = 0x%x",
			sectors_per_cluster_bits);
	nr_hidden_sects = le32_to_cpu(b->bpb.hidden_sectors); //Òþ²ØÉÈÇøÊý
	ntfs_debug("number of hidden sectors = 0x%x", nr_hidden_sects);
	vol->cluster_size = vol->sector_size << sectors_per_cluster_bits; //´Ø´óÐ¡
	...
	clusters_per_mft_record = b->clusters_per_mft_record; //mft recordËùÕ¼µÄ´ØÊý
	...
	if (clusters_per_mft_record > 0)
		 ...
		 */
		vol->mft_record_size = 1 << -clusters_per_mft_record; //mft record´óÐ¡:1024
	...
	clusters_per_index_record = b->clusters_per_index_record;	//index recordËùÕ¼µÄ´ØÊý : 1
	...
	vol->index_record_size_mask = vol->index_record_size - 1;//INDEX record´óÐ¡: 4096
	...
	vol->mft_lcn = ll;//mft dataµÄÊ×´Ø£¬¼´ÏµÍ³ÎÄ¼þ$MFT
	...
	vol->mftmirr_lcn = ll;//ÏµÍ³ÎÄ¼þ$mftmirr µÄÊ×´Ø
	...
	ntfs_debug("vol->mftmirr_size = %i", vol->mftmirr_size);//ÏµÍ³ÎÄ¼þ$mftmirr µÄÏîÊý:4 
#endif /* NTFS_RW */
	vol->serial_no = le64_to_cpu(b->volume_serial_number);//¾í±êÐòÁÐºÅ
...
static void ntfs_setup_allocators(ntfs_volume *vol)
{
	...
	mft_zone_size = vol->nr_clusters;	//MFT µÄ´óÐ¡
	...
	ntfs_debug("vol->mft_zone_pos = 0x%llx",	//MFT ÆðÊ¼´ØºÅ
	...
	ntfs_debug("vol->mft_zone_end = 0x%llx",	//MTF½áÊø´ØºÅ
	...
	vol->mft_data_pos = 24;
	ntfs_debug("vol->mft_data_pos = 0x%llx",		//MFT Ç°24¸öÎªÏµÍ³ÎÄ¼þ£¬Ö®ºó²Å´æ´¢ÓÃ»§Êý¾ÝµÄMFT 
...
static bool load_and_init_mft_mirror(ntfs_volume *vol)
{
	...
	NInoSetMstProtected(tmp_ni);	//$MFTMirr,  $MFTÊý¾Ý¶¼ÐèÒªÐÞÕýÖµÐÞÕý£¬ÇÒ²»Ö§³ÖÏ¡Êè
...
static bool check_mft_mirror(ntfs_volume *vol)
{
	...
	mrecs_per_page = PAGE_CACHE_SIZE / vol->mft_record_size;//Ã¿pageµÄmft ¼ÇÂ¼Êý
...
mft_unmap_out:
	...
	/* Construct the mft mirror runlist by hand. */ //´´½¨mft mirrorµÄrunlistå
	rl2[0].vcn = 0;
...
static bool load_and_init_quota(ntfs_volume *vol)
{
	...
	vol->quota_ino = tmp_ino;		//$Extend/$Quota/$Q
 ...
 * Return 'true' on success or 'false' on error.
 */
 //µ÷ÓÃÖ®Ç°mountÊÇ¶ÁÐ´·½Ê½£¬¶ø²»ÊÇÖ»¶ÁµÄ
 
 void change_to_readonly(struct super_block *sb)
{
	ntfs_warning(sb,"change to read only ");
...
static bool load_system_files(ntfs_volume *vol)
{
	...
	vol->mftbmp_ino = ntfs_attr_iget(vol->mft_ino, AT_BITMAP, NULL, 0);//µÃµ½$MFT:bitmapµÄinode
	...
	/* Read upcase table and setup @vol->upcase and @vol->upcase_len. */
	if (!load_and_init_upcase(vol))	//µÃµ½$UPCASE£¬²¢³õÊ¼»¯@vol->upcase and @vol->upcase_len
	 ...
	 * @vol->attrdef_size.
	 */
	if (!load_and_init_attrdef(vol))	//µÃµ½$attdef ,²¢³õÊ¼»¯vol->attrdef
...
iput_volume_failed:
	...
	//0x70 AT_VOLUME_INFORMATION Ö»ÊÇÏµÍ³ÎÄ¼þ$volumeµÄÌØ¶¨ÊôÐÔ
	if (ntfs_attr_lookup(AT_VOLUME_INFORMATION, NULL, 0, 0, 0, NULL, 0,
			ctx) || ctx->attr->non_resident || ctx->attr->flags) {//0x70 ²»ÄÜÎª·Ç×¤Áô£¬²»¿ÉÑ¹Ëõ¡¢Ï¡Êè»ò¼ÓÃÜ
err_put_vol:
		ntfs_attr_put_search_ctx(ctx);
get_ctx_vol_failed:
	...
	printk(KERN_INFO "NTFS volume version %i.%i.\n", vol->major_ver,
			vol->minor_ver);
	if (vol->major_ver < 3 && NVolSparseEnabled(vol)) { //NTFS 3.0ÒÔÏÂµÄ°æ±¾²»Ö§³ÖÏ¡ÊèÌØÐÔ
	...
	if (vol->vol_flags & VOLUME_MUST_MOUNT_RO_MASK) {
		...
		if (!(sb->s_flags & MS_RDONLY)) {
			...
			//change_to_readonly(sb);
			ntfs_error(sb, "%s.  Mounting read-only%s, changed, may bug", es1, es2);//ÎªÉ¶°¡?
		} else
			ntfs_warning(sb, "%s.  Will not be able to remount "
	...
	rp = NULL;
	//$logfile Ò²²»ÄÜÏ¡Êè
	if (!load_and_check_logfile(vol, &rp) ||!ntfs_is_logfile_clean(vol->logfile_ino, rp)) { 
		...
		if (!(sb->s_flags & MS_RDONLY)) {
			...
			//change_to_readonly(sb);
			ntfs_error(sb, "%s.  Mounting read-only%s, changed, may bug", es1, es2);
			goto modify2;
		} else
			ntfs_warning(sb, "%s.  Will not be able to remount "
					"read-write%s", es1, es2);
...
modify2:
	...
	vol->root_ino = ntfs_iget(sb, FILE_root); 		//¸ùÄ¿Â¼!!!!
...
#if 0
	 //ÔÚ¶ÁÐ´ÉõÖÁ´´½¨²Ù×÷Ê±²¢Ã»¸ÄÐ´&logfile,Òò´ËÒ²Ã»±ØÒªÇå¿ÕËü
	if (!(sb->s_flags & MS_RDONLY) && !ntfs_empty_logfile(vol->logfile_ino)) {
...
static int ntfs_fill_super(struct super_block *sb, void *opt, const int silent)
{
	...
	*vol = (ntfs_volume) {
	...
	NVolSetSparseEnabled(vol);//Ö§³ÖÏ¡ÊèÎÄ¼þ!
	 ...
	 */
	blocksize = sb_min_blocksize(sb, NTFS_BLOCK_SIZE);//¿é´óÐ¡Îª512
	...
	vol->nr_blocks = i_size_read(sb->s_bdev->bd_inode) >>//Éè±¸µÄ¿éÊý
			sb->s_blocksize_bits;
	/* Read the boot sector and return unlocked buffer head to it. */
	if (!(bh = read_ntfs_boot_sector(sb, silent))) {	//¶ÁÈ¡Òýµ¼ÉÈÇø
		if (!silent)
	 ...
	 * using it.
	 */
	result = parse_ntfs_boot_sector(vol, (NTFS_BOOT_SECTOR*)bh->b_data);//½âÎöÒýµ¼ÉÈÇø
	brelse(bh);
	if (!result) {
		if (!silent)
	...
	ntfs_setup_allocators(vol);//³õÊ¼»¯MFT¼ÇÂ¼ºÍÊý¾ÝµÄ·ÖÅäÇé¿ö
