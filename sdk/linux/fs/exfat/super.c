...
enum { Opt_uid, Opt_gid, Opt_fmode, Opt_dmode, Opt_fmask,Opt_dmask,Opt_nls, Opt_err, };
			 ...
			 int is_remount)
{
	...
	opts->nls	= load_nls_default();//UTF8
	...
	while ((p = strsep(&data, ",")) != NULL) {
		...
		case Opt_uid:
			if (is_remount) {
				if (!silent)
					exfat_error("exFAT: cannot change"
		...
		case Opt_nls:
			...
			if (!opts->nls) {
				if (!silent)
					exfat_error("exFAT: couldn't load "
					       " nls \"%s\"", nls_str);
			}
			kfree(nls_str);
			if (!opts->nls)
		...
		default:
			if (!silent) {
				exfat_error( "exFAT: Unrecognized mount"
				       " option \"%s\" or missing value", p);
			}
			goto error;
		}
	}
...
static int exfat_remount(struct super_block *sb, int *flags, char *data)
{
	...
	if (!sbi->upcase && remount_rw) {
		exfat_error("exFAT: upper-case table isn't available,"
...
static void exfat_write_super(struct super_block *sb)
{
	...
	if ((sb->s_flags & MS_RDONLY)){
		exfat_error("the filesystem has been set to RDONLY");
		return;
	}
	bh = sb_bread(sb, 0);//读取物理0扇区
	if (!bh) {
		exfat_error("exFAT: unable to read sector 0");
		return;
	}
	exsb = (struct exfat_super_block *)bh->b_data;
	exsb->allocated_percent = (sbi->total_clusters-sbi->free_clusters)/sbi->total_clusters;
...
error_io:
	exfat_error("exFAT: couldn't read super block to verify");
	return -EIO;

error_checksum:
	if (!silent)
		exfat_error("exFAT: checksum failed for super block");
	return -EINVAL;
}
...
/*	sb->s_export_op = &exfat_export_ops; FIXME: don't support nfs? */
	...
	if (!sb_min_blocksize(sb, EXFAT_MIN_BLKSIZE)) { //将blocksize设为最小:512
		exfat_error("exFAT: unable to set block size");
		err = -EINVAL;
		goto error;
	}
	...
	sbi->fat_blocknr = le32_to_cpu(exsb->fat_blocknr);//FAT1的起始扇区
	sbi->fat_block_counts = le32_to_cpu(exsb->fat_block_counts);//FAT1的总扇区数
	sbi->clus_blocknr = le32_to_cpu(exsb->clus_blocknr);//数据区的起始簇号
	sbi->total_clusters = le32_to_cpu(exsb->total_clusters);//总扇区数
	sbi->rootdir_clusnr = le32_to_cpu(exsb->rootdir_clusnr);//根目录的起始簇号
	sbi->state = le16_to_cpu(exsb->state);//卷标的状态
	sbi->bpc_bits = exsb->block_per_clus_bits;
	sbi->bpc = 1 << exsb->block_per_clus_bits;//每簇的扇区数
	sbi->serial_number = le32_to_cpu(exsb->serial_number);//序列号

	err = exfat_exsb_validate(sb, exsb, silent);//分区引导扇区数据的合法性
	if (err)
		goto error;
...
#else
		exfat_error("exFAT: blocksize is not same with"
		       " sector size");
		goto error;
#endif
	}
	...
	err = parse_options(&sbi->opts, data, silent, 0);//解析挂载参数
	if (err)
		goto error;
	...
	if (sbi->state & EXFAT_SB_DIRTY) {
		exfat_waring("exFAT: filesystem is not clean");
		/* FIXME: we need to do something? */
	}

	sb->s_root = d_alloc_root(root_inode);//将根目录节点转为dentry
	if (!sb->s_root) {
		exfat_error("exFAT: couldn't get root dentry");
		goto error_iput;
	}
