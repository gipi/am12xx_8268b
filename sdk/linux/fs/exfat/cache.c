...
struct exfat_cache {
	...
	u32	iclusnr;		/* cluster number on data of inode. 当前数据的逻辑簇号*/
	u32	clusnr;			/* cluster number on disk. 物理簇号*/
	u32	len;			/* number of contiguous clusters 连续簇数*/
};
					  ...
					  struct exfat_cache *cache)
{
	...
	if (exi->cache_lru.next != &cache->cache_list)
		list_move(&cache->cache_list, &exi->cache_lru);//移除cache->cache_list
}

/*fid the exfat_cache of 'iclusnr' or nearest chache, and use it to initialize @cid*/
		       ...
		       struct exfat_cache_id *cid)
{
	...
	list_for_each_entry(p, &exi->cache_lru, cache_list) {
		/* Find the cache of "iclusnr" or nearest cache. *///找到iclusnr对应的cache或者与其最近的
...
void exfat_cache_add(struct inode *inode, struct exfat_cache_id *new)
{
	...
	if (cache == NULL) {
		if (exi->nr_caches < exfat_max_cache(inode)) {//8
			exi->nr_caches++;
			spin_unlock(&exi->cache_lru_lock);
