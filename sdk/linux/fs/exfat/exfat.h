...
#define EXFAT_MODIFY /*all the modify work @by yangli*/
/* FIXME: remove this */
...
#define EXFAT_DE_MAX_CHUNKS	(1 + EXFAT_DE_MAX_SUB_CHUNKS) //256
#define EXFAT_DE_MAX_SIZE	(EXFAT_DE_MAX_CHUNKS << EXFAT_CHUNK_BITS) //256*32
#define EXFAT_DE_MAX_BH \
	(DIV_ROUND_UP(EXFAT_DE_MAX_SIZE, EXFAT_MIN_BLKSIZE) + 1)//(256*32+512-1)/512+1
...
struct exfat_sb_info {
	...
	u32		last_cluster;		/*last allocated cluster*/
...
struct exfat_inode_info {
	...
	loff_t		phys_size;		/* physically allocated size文件总大小 */

	u32		clusnr;	//起始的物理簇号
	u32 		last_clusnr;//最后分配的物理簇号
	u32         iclusnr;	//总簇数
	__le16		attrib;
	u8		data_flag;
	u8 		dataChangeFlag;
	 ...
	 * faster way */
	sector_t	de_blocknr[EXFAT_DE_MAX_BH];/* blocknr of this entry : 该entry(目录项)所在的物理扇区号,目录项可以跨越多个扇区*/
	unsigned long	de_offset;		/* start offset in buffer 	:该目录项在所在扇区的偏移*/
	unsigned long	de_size;		/* size of this entry : 该目录项的总大小*/
...
struct exfat_entry_data {
	unsigned long	size;				/* size of this entry */

	sector_t  bnr[EXFAT_DE_MAX_BH];/* blockes number for this entry */
	unsigned long	bh_offset;		/* start offset in buffer */
	int		nr_bnr;				/* number of blockes */
};
...
extern u64 exfat_div_roundup(u64 dividend, u64 divisor);
...
#ifdef EXFAT_MODIFY
extern int exfat_write_inode(struct inode *inode, int wait);
extern int exfat_sync_inode(struct inode *inode);
extern void exfat_unmap_page(struct page *page);
extern struct page *exfat_map_page(struct address_space *mapping,
		unsigned long index);
extern int exfat_add_cluster(struct inode *inode);
extern  void exfat_Print_Buf(unsigned char * title,  const unsigned char * pData, unsigned int inLen);
extern struct inode *exfat_build_inode(struct super_block *sb,struct exfat_entry_data *ed,u16 attrib , u64 size, u32 dclusnr);
extern void exfat_attach2(struct inode *inode, struct exfat_entry_data *ed);
