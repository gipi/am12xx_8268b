	...
	*nr_slots = 0;
	...
	if(is_dir == 2){
		...
		if(l> (11+7))
		{
			...
			len -= 7;
		memcpy((void *)name,(void *)(name+7),l);
		memcpy(msdos_name,name,11);
		de = (struct msdos_dir_entry *)slots;
		err = 0;
		printk("volume name:%s,len :%d\n",msdos_name,len);
		goto shortname;
				  ...
				  struct nameidata *nd)
{
	...
	/*printk("vfat:Looking up %s in directory inode 0x%lx.\n",
...
static int vfat_mkdir(struct inode *dir, struct dentry *dentry, int mode)
{
	...
	if((dir->i_ino == MSDOS_ROOT_INO) && !strncmp(dentry->d_name.name,"VOLUME_",7))
	{
		unsigned int len = dentry->d_name.len;	 
		len -= 7;
		if(len > 11)
		len  =11;
		printk("creating  volume !,name :%s, len :%d\n",dentry->d_name.name,len);
		if(find_volume(dir,dentry->d_name.name,len))
		{		
			unlock_super(sb);
			return 0;
		}
	...
	err = vfat_add_entry(dir, &dentry->d_name, is_volume, cluster, &ts, &sinfo);
	if (err)
		goto out_free;
	if(is_volume == 2)
		goto out;
