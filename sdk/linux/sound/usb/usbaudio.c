...
#define TOTAL_TIME 10  //s
static long long lasttime=0;
static unsigned int lastdacpos=0;
static int count=44100*(TOTAL_TIME-1);
#endif
...
#ifdef ONLYDEBUG
	long long	time;	
	int elapstime,datatime,dactime;	
	unsigned int dacpos;
#endif
...
#ifdef ONLYDEBUG
		count+=len;
		if(count>=44100*TOTAL_TIME)
		{
			dacpos=44100-(*(volatile unsigned int *)0xb0100018)&0xffff; //dac pos
			time=us_gettimeofday();

			elapstime=((int)(time-lasttime))/1000;
			datatime=count*1000/44100;
			if(dacpos>lastdacpos)
			{
				dactime=(dacpos-lastdacpos)*1000/44100+	TOTAL_TIME*1000;
				printk("dactime=%dms,elapstime:%dms,datatime=%dms,dac empty=%dms\n",dactime,elapstime,datatime,dactime-datatime);
			}
			else
				printk("elapstime:%dms,datatime=%dms,delay=%dms\n",elapstime,datatime,elapstime-datatime);
...
static int snd_usb_create_streams(struct snd_usb_audio *chip, int ctrlif)
{
	...
	int i, j;
	int android_patch=0;
    struct usb_host_interface *alts2;
    struct usb_interface_descriptor *altsd2;
	...
	/**
    * patch for andorid accessory + audio mode.
    */
    if(chip->usb_id == USB_ID(0x18d1,0x2d04) || chip->usb_id == USB_ID(0x18d1,0x2d05)){
        if( (p1[7]==2) && (p1[8]==0) && (p1[9]==1)){
            iface = usb_ifnum_to_if(dev, 0);
            alts2 = &iface->altsetting[0];
		    altsd2 = get_iface_desc(alts2);
            if( altsd2->bInterfaceClass == USB_CLASS_VENDOR_SPEC && \
                altsd2->bInterfaceSubClass == USB_CLASS_VENDOR_SPEC){
                android_patch = 1;
            }
        }
    }
