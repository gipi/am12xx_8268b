...
extern struct am7x_de_isr_data  de_isr_data;
/*****************************************************/
/*     公用函数部分*/
/*****************************************************/
...
void EnterFrameOver(OS_CPU_SR* pFlag)
{
	...
	if(RegBitRead(DE_Con,9,9)==1)
	{
		...
		if(mode==0x0)//lcd:rgb
		{
			#if((CONFIG_AM_CHIP_ID == 1201)||(CONFIG_AM_CHIP_ID == 1205)||(CONFIG_AM_CHIP_ID == 1203)||(CONFIG_AM_CHIP_ID == 1207))
			...
			if ((act_readl(DB_WR)==0x1) && (de_isr_data.de_isr_init_flag == 1))
			{
				de_isr_data.de_lamp=1;
				//printk("\npend\n");
				down(&(de_isr_data.de_sem));
			}			
			#endif
		...
		}else//bt
		{
				...
				while(RegBitRead(DE_INT,2,2)==0);
			#elif((CONFIG_AM_CHIP_ID == 1211)||(CONFIG_AM_CHIP_ID == 1220)||(CONFIG_AM_CHIP_ID == 1213))
			if (de_isr_data.de_isr_init_flag == 1)
			{
				de_isr_data.de_lamp=1;
				down(&(de_isr_data.de_sem));
			}
...
//		GammaSet(1,(INT32U *)(display->Gamma));
	...
	act_writel(0x1d0002,	OSD0_FIFO_Thrd);//0x1f0008
	act_writel(0x1d0002,	OSD1_FIFO_Thrd);
	//act_writel(0x330033,		0xb0040000+0x38);
	//act_writel(0x0,		0xb0040000+0x3c);
	act_writel(0xf0002,	OSD0_RDY_Thrd);//0x30002
	act_writel(0xf0002,	OSD1_RDY_Thrd);
	#endif
	#if (CONFIG_AM_CHIP_ID == 1220) || (CONFIG_AM_CHIP_ID == 1213)
	act_writel(0x3d0010,	OSD0_FIFO_Thrd);//0x1f0008
	act_writel(0x3d0010,	OSD1_FIFO_Thrd);
	act_writel(0x3f003f,	Frm_FIFO_Thrd);
	act_writel(0x3e,		Frm_state_Thrd);
	act_writel(0xf0002,	OSD0_RDY_Thrd);//0x30002
	act_writel(0xf0002,	OSD1_RDY_Thrd);
	#endif	
...
INT32S  ImageSet(OutImageAttribute* outimage, InImageAttribute* inimage,DisplayAttribute* display)
{
	...
	}else
	{
		...
		#endif
		//WINDOWS
		if ((display->Mode == RGBMODE_I) || (display->Mode == BTMODE_I))
		{
			act_writel((outimage->OutImageXStart<<16)|(outimage->OutImageYStart/2),Win_Coor1);
			act_writel((outimage->OutImageXEnd<<16)|(outimage->OutImageYEnd/2-1),Win_Coor2);
		}
		else
		{
...
//	#elif ((CONFIG_AM_CHIP_ID == 1203)||(CONFIG_AM_CHIP_ID == 1207)||(CONFIG_AM_CHIP_ID == 1211))
//		for(i=0;i<16;i++)
//		{
...
void Pallet16Set(	INT32S * Pallet,	INT32S  TransColor)
{
	...
	EnterFrameOver(&flag);		
	for(i=0;i<16;i++)	
		act_writel(*(Pallet+i),Pallet1Ram);
	ExitFrameOver(&flag);
}
...
/*	
	...
	printf("csc->e=%d\n",csc->e);
	printf("csc->ysub16=%d\n",csc->ysub16);
	printf("csc->color_coef=%d\n",csc->color_coef);
	printf("csc->offset1=%d\n",csc->offset1);
	printf("csc->offset2=%d\n",csc->offset2);
	printf("csc->contrast1=%d\n",csc->contrast1);
	printf("csc->contrast2=%d\n",csc->contrast2);
*/	
...
void  DisplayClkDisable(void)
{
	...
	RegBitSet(0,CMU_LCDPLL,0,0);		//LCD PLL enable 1:enable,0:disable 注意1201 spec有误
	ExitFrameOver(&flag);
}
...
{
	RegBitSet(cursor->SW,DE_Con,21,21);
	RegBitSet(cursor->x,CursorCoor,10,0);
	RegBitSet(cursor->y,CursorCoor,26,16);
	act_writel(0x1,DB_WR);
}
