...
int map_nopage(struct vm_area_struct *vma, struct vm_fault *vmf);
...
int map_nopage(struct vm_area_struct *vma, struct vm_fault *vmf)
{
 ...
 return -1;
  ...
  atomic_inc(&(virt_to_page((void *)virt_addr)->_count));
  printk("map_drv: page fault for offset 0x%lx (kseg x%lx)\n",offset, virt_addr);
vmf->page = virt_to_page((void *)virt_addr);
