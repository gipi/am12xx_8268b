...
void fb_edid_to_monspecs(unsigned char *edid, struct fb_monspecs *specs)
{
	...
	if (!(edid_checksum(edid)))
	{
		DPRINTK("edid_checksum error\n");
		return;
	}

	if (!(edid_check_header(edid)))
	{
		DPRINTK("edid_check_header error\n");
		return;
	}
	...
	specs->extensionflag = edid[EDID_EXTENSION_FLAG];

	DPRINTK("========================================\n");
	DPRINTK("Display Information (EDID Block0)\n");
	DPRINTK("========================================\n");
...
void fb_edid_add_monspecs(unsigned char *edid, struct fb_monspecs *specs)
{
	...
	DPRINTK("========================================\n");
	DPRINTK("Extension Display Information (EDID Block1)\n");
	DPRINTK("========================================\n");
	...
	while (pos < edid[2]) {
		...
		}else if (type == 3){//Vendor Specific Data Block
			if (edid[pos+1]==0x03 && edid[pos+2]==0x0c && edid[pos+3]==0x00){
				DPRINTK(" HDMI VSDB\n");
				/* HDMI IEEE Registration Identifier */
				phys_addr = (edid[pos+4]<<8) + edid[pos+5];
				DPRINTK("found physical address %04X\n", phys_addr);
				specs->vsdbflag = 1;
			}
		}
		pos += len + 1;
