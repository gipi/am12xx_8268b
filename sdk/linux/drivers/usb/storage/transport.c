	...
	void *buf, unsigned int length, unsigned int *act_len)
{
	...
	if (act_len){	
		*act_len = us->current_urb->actual_length;
		US_DEBUGP("%s: act_len :%d,result:%d\n",__FUNCTION__,*act_len,result);
	}	
...
void usb_stor_invoke_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	if ((scsi_get_resid(srb) > 0) &&
		...
		US_ERR("-- unexpectedly short transfer\n");
	}
...
int usb_stor_Bulk_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	//printk(KERN_ALERT "Bulk command transfer result=%d\n", result);
	if (result != USB_STOR_XFER_GOOD){
	...
	if (transfer_length) {
		...
		if (result == USB_STOR_XFER_ERROR){
			US_ERR("Bulk data transfer result 0x%x\n", result);
			return USB_STOR_TRANSPORT_ERROR;
		}
	...
	if (result == USB_STOR_XFER_STALLED) {
		...
		US_ERR("Attempting to get CSW (2nd try)...\n");
		result = usb_stor_bulk_transfer_buf(us, us->recv_bulk_pipe,
	...
	if (result != USB_STOR_XFER_GOOD){
		print_hex_dump(KERN_ALERT, "wrong csw:", DUMP_PREFIX_OFFSET,
				16, 1, bcs, cswlen, 0);
		return USB_STOR_TRANSPORT_ERROR;
	}
	/* check bulk status */
