...
{	
	...
	for(i = 1;i < ARRAY_SIZE(hcep_map);i++){	
		...
		DMSG_HCD("<am7x_hcd>@@@---find ep,mask:%x,type:%x\n",ep->mask,ep->type);
		return ep;
	}	
	return NULL;
}
...
#if 0  //to config ep3 buffer ---quad buffer(ep x in control register bit[0:1])
...
static void read_hcep0_fifo(struct aotg_hcep *ep, struct urb *urb)
{
	...
	if(aotg_readb(HCEP0CS) & EP0CS_HCOUTBSY) {
		DMSG_CTRL("<am7x_hcd>@@@---<CTRL>IN data is not ready\n");
		return ;
	}else {
		...
		DMSG_CTRL("<am7x_hcd>@@@---<CTRL>IN : receive %dB %s %s %s %s actual %dB claim %dB\n",
...
static void handle_hcep0_out(struct aotg_hcd *acthcd)
{
	...
	if(list_empty(&ep->hep->urb_list)) {
		urb = NULL;
		DMSG_CTRL("<am7x_hcd>@@@---<CTRL> out irq come,but no urb\n");
		return;
	}
...
static void handle_hcep0_in(struct aotg_hcd *acthcd)
{
	...
	if(list_empty(&ep->hep->urb_list)) {
		urb = NULL;
		DMSG_CTRL("<am7x_hcd>@@@---<CTRL> in irq come,but no urb\n");
		return;
	}
...
hcin0err_exit:
		...
		default:
			DMSG_HCD("<am7x_hcd>@@@---aotg_hcd doesn't care this IRQ 0x%x\n",irqvector);
			break;
	}
...
start_work:	
	if (unlikely(list_empty(&ep->hep->urb_list))) {
		DMSG_HCD("<am7x_hcd>@@@---ep:%x,urb list empty\n",ep->mask);
		return 0;
	}
	...
	case PIPE_BULK:
		...
		#ifdef DEBUG_TXRX
				INFO("<am7x_hcd>@@@---<ASYNC XFERS>bulk ep %x out busy,cs(0x%x):%x,con(0x%x):%x\n",
					ep->mask,
					ep->reg_hcepcs,
					aotg_readb(ep->reg_hcepcs),
					ep->reg_hcepcon,
...
static void start_schedule(struct aotg_hcd *acthcd,struct aotg_hcep* ep )
{
	...
	DMSG_XFERS("<am7x_hcd>@@@---start schedule......\n");
	if ( !ep->period)
		start_async_transfer(acthcd,ep);
	else
...
*/	
	...
	if (hep->hcpriv) {/*ep has been initialized*/
		...
		case PIPE_CONTROL:
			if(!acthcd->ep_list[0]) {
				...
				ERR("<am7x_hcd>@@@---<QUEUE> initialize ep0 failed,something odd\n");
				retval = -ENOSPC;
				goto error;
			}
			break;
		...
		case PIPE_INTERRUPT:
			...
			if(retval < 0) {
				ERR( "<am7x_hcd>@@@---<QUEUE>cannot config period ep\n");
				goto error;
			}
	...
	switch(type) {
		case PIPE_CONTROL:	
           		if (list_empty(&ep->schedule)){
				DMSG_CTRL("<am7x_hcd>@@@---<QUEUE> add control ep%s%d to schedule queue\n",is_out ?"out":"in",epnum);
		...
		case PIPE_BULK:	
			if(ep->stalled){
				INFO("<am7x_hcd>@@@---pipe is stalled,clear before tranfer\n");
				ep->stalled = 0;
				aotg_hcep_reset(ep->mask,ENDPRST_FIFOREST |ENDPRST_TOGRST);			
			}	

			if (list_empty(&ep->schedule)){
				DMSG_XFERS("<am7x_hcd>@@@---<QUEUE> bulk ep%s%d  is added in the schedule queue\n",is_out ?"out":"in",epnum);
				list_add_tail(&ep->schedule, &acthcd->async);	
				pio_irq_enable(ep->mask);
				validate_ep(ep);
		#ifdef DEBUG_TXRX			
				INFO("<am7x_hcd>@@@---ep not in async queue,epnum:%x,epmask:%x,epcs:%x,epcon:%x\n",
							ep->epnum,
							ep->mask,
							aotg_readb(ep->reg_hcepcs),
							aotg_readb(ep->reg_hcepcon));
				...
				validate_ep(ep );
		#ifdef  DEBUG_TXRX
					INFO("<am7x_hcd>@@@---ep exist in async queue,epnum:%x,epmask:%x,epcs:%x,epcon:%x\n",
							ep->epnum,
							ep->mask,
							aotg_readb(ep->reg_hcepcs),
							aotg_readb(ep->reg_hcepcon));
		#endif
				DMSG_XFERS("<am7x_hcd>@@@---<QUEUE> ep%s%d exists in the schedule queue\n",is_out ?"out":"in",epnum);	
			}
			break;
		...
		case PIPE_ISOCHRONOUS:
			...
			if (retval < 0){
				ERR("<am7x_hcd>@@@---cannot balance use this period\n");
				goto error;
			}
		 	...
		 	*/	
			DMSG_HCD("<am7x_hcd>@@@---schedule qh%d/%p branch %d\n", ep->period, ep, ep->branch);
			for (i = ep->branch; i < PERIODIC_SIZE; i += ep->period) {
				struct aotg_hcep	**prev = &acthcd->periodic[i];
...
static int aotg_hcd_urb_dequeue(
	...
	if (ep) {
		if (ep->hep->urb_list.next != &urb->urb_list) {
			DMSG_XFERS("<am7x_hcd>@@@---<DEQUEUE> this urb belongs to ep%d is not active",ep->epnum);
		}else if(ep != acthcd->next_async){
			DMSG_XFERS("<am7x_hcd>@@@---<DEQUEUE> this ep%d is not active",ep->epnum);
		}
		else {
                      urb->status = -EPIPE;//for prevent hcd fatal error when irq lost
			DMSG_XFERS("<am7x_hcd>@@@---<DEQUEUE> urb is active, wait for irq\n");
		}
...
static void aotg_hcd_endpoint_disable(
	...
	DMSG_HCD("<am7x_hcd>@@@---disable ep:%d\n",hep->desc.bEndpointAddress);
	if (!ep)
		return;
	...
	DMSG_HCD("<am7x_hcd>@@@---<EP DISABLE> remove ep%d index %d from global ep list\n",ep->epnum,index);
	ep->inuse = 0;
	hep->hcpriv = NULL;
