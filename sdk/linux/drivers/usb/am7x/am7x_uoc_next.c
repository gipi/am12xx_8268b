...
static struct cdev uoc_next_dev;
#endif
...
static unsigned int detect_gpio = 95;
static int is_otg = 0;
...
static int usb_cur_inout_status_next = USB_CUR_STATUS_PLUG_OUT;
#endif
...
static const char debug_filename[] = "driver/uoc1";
static int proc_uoc_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "\nFSM: 0x%02x,OTGSTATUS:0x%02x USBEXTCTRL:0x%02x USBESTA:0x%02x\n", 
		aotg_readb(OTGSTATE) & 0x0f,
		aotg_readb(OTGSTATUS),
		aotg_readb(USBEXTCTRL),
		aotg_readb(USBESTA));
	
	seq_printf(s, "\nOTGIEN: 0x%02x: USBIEN:0x%02x,USBCS:0x%02x,OTGCTRL:0x%02x\n",
		aotg_readb(OTGIEN),
		aotg_readb(USBIEN),
		aotg_readb(USBCS),
		aotg_readb(OTGCTRL));
	...
	seq_printf(s,"\n********************************\n");
	seq_printf(s, "\nportstatus: 0x%x: portchange:0x%x\n",
		ptrans->port_status,
		ptrans->port_change);
...
static inline  int test_usb_clk(void)
{
	...
	return ((aotg_readl(CMU_DEVCLKEN2)  & (1<<1  | 1<<2)) !=0 );
#else
	return ((aotg_readl(CMU_DEVCLKEN)  & (USBOTG_EN  | USBPHY_EN)) !=0 );
#endif
}
...
static  int  usb_clk_open(void)
{
	...
	aotg_writel(aotg_readl(CMU_DEVCLKEN2) |1<<1 |1<<2 ,
			CMU_DEVCLKEN2);
	#else
	aotg_writel(aotg_readl(CMU_DEVCLKEN) |USBOTG_EN |USBPHY_EN ,CMU_DEVCLKEN);
	#endif
	for(cnt =0;cnt <10;cnt++){
		mdelay(5);
		uoc_set_phy(0x95,0);
//		printk("debug---%s %d vdstat=%02x\n",__FUNCTION__,__LINE__,aotg_readb(VDSTAT));
...
{
	#if CONFIG_AM_CHIP_ID==1213
	aotg_writel((aotg_readl(CMU_DEVCLKEN2) & ~(1<<1 |1<<2)),CMU_DEVCLKEN2);
	#else
	aotg_writel((aotg_readl(CMU_DEVCLKEN) & ~(USBOTG_EN |USBPHY_EN)),CMU_DEVCLKEN);
	#endif
}
...
{	
	...
	#else
	aotg_writel((aotg_readl(CMU_DEVRST2)&0xfffffff9),CMU_DEVRST2);
	mdelay(10);
	aotg_writel((aotg_readl(CMU_DEVRST2) |0x06),CMU_DEVRST2);
	#endif
}
#ifdef UOC_FILE_OPERATION
static enum{
...
static int aotg_uoc_next_open(struct inode *inode, struct file *fd)
{
	int ret = 0;
...
static int aotg_uoc_next_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
...
static int aotg_uoc_next_release(struct inode *inode, struct file *fd)
{
	int ret= 0;
...
static struct file_operations uocnext_operations = {
	.owner =	THIS_MODULE,
	.open 			=	aotg_uoc_next_open,
	.ioctl 	= 	aotg_uoc_next_ioctl,
	.release 			=	aotg_uoc_next_release
};
#endif
...
static int dev_pattern_check(struct aotg_uoc *_aotg)
{
	...
	save_phy_ctl = aotg_readl(USB2_PHY_CTL);//bq:usb2_phy_ctl 1213 spec 473page
	/*discon DP,DM 500k ohm pull up resistor*/
	aotg_writel(aotg_readl(USB2_PHY_CTL)&0xfffffff0,USB2_PHY_CTL);
	...
	if(val!=0)
	{
		DMSG_UOC("reset linestate0,1 error\n");
		val = -1;
		goto exit;
	}

	/*discon 15k ohm Rpd for DP,DM*/
	aotg_writel(aotg_readl(USB2_PHY_CTL)|0x3,USB2_PHY_CTL);

	/*pull up 500k ohm Rpu for DM for charge*/
	aotg_writel(aotg_readl(USB2_PHY_CTL)|0x8,USB2_PHY_CTL);
	mdelay(30);

	/*pull down 500k ohm to normal state*/
	aotg_writel(aotg_readl(USB2_PHY_CTL)&0xfffffff7,USB2_PHY_CTL);
	mdelay(5);
	...
	switch(val)
	{
		case 0x00:
			ret  = TYPE_PC_CONNECT;
			INFO("PC connect mode\n");
			break;
		case 0x02:
			ret  = TYPE_PC_NODPDM;
			INFO("power adapter mode\n");
			break;
		case 0x03:
			ret  = TYPE_ADP_CHARGE;
			INFO("battery charge mode\n");
			break;
		default:
			ret  = TYPE_NO_SUPPORT;
			INFO("error line state\n");
			break;
	}
exit:	
	/*enable 1.5k ohm Rpu*/
	...
	aotg_writel(save_phy_ctl,USB2_PHY_CTL);
...
static void check_state(struct aotg_uoc *uoc, const char *tag)
{
	...
	DMSG_UOC("otg: %s HARDFSM %s/%02x,transc_state %s ,orignal=%s\n",\
		tag,
		state_string(state),
		fsm,
	...
	switch(state) {
		...
		case OTG_STATE_B_PERIPHERAL:
			if(!uoc->transceiver.gadget){
				DMSG_UOC("<RUN_FSM>update state ,pull down to hide\n");
				dplus_down();
			}
			break;		
		default:
			DMSG_UOC("<RUN_FSM> OTG has already run,state is %02x,do nothing\n",state);
			break;
	}
#endif
	DMSG_UOC("OTGIRQ %02x,id %d, OTGSTATE %02x\n",aotg_readb(OTGIRQ),id,state);
}


static void auto_power_on(struct aotg_uoc *uoc)
{
	DMSG_UOC("<POWER> power on\n");
...
static void aotg_a_idle(struct aotg_uoc * uoc)
{
	...
	switch(uoc->prevstate) {
		case OTG_STATE_A_WAIT_BCON:
			if(is_a_bus_drop) {
				DMSG_UOC("<A_IDLE>state transfer, caused by a_bus_drop\n");
			}
		//	auto_power_on(uoc);
			break;
		case OTG_STATE_A_SUSPEND:
			if(uoc->transceiver.host){
				DMSG_UOC("<SUPEND>notify for a suspend\n");
				port_event_inform(uoc,AOTG_VBUS_DROP_EVENT);
			}else 
				DMSG_UOC("<A_IDLE>no notify handler\n");
			if(is_a_bus_drop) {
				auto_power_on(uoc);
				DMSG_UOC("<A_IDLE>state transfer, caused by a_bus_drop\n");	
			}
...
static void aotg_a_host(struct aotg_uoc *uoc)
{
	...
	switch(uoc->prevstate) {
		case OTG_STATE_A_WAIT_BCON:
			if(is_b_conn) {
				if(uoc->transceiver.host){
					DMSG_UOC("<A_HOST>plug in\n");
					port_event_inform(uoc,AOTG_PLUGIN_EVENT);
				}
				else{
					DMSG_UOC("<A_HOST>no notify handler\n");
				}
				...
				DMSG_UOC("<A_HOST> odd b_conn status\n");
			break;
		...
		case OTG_STATE_A_HOST:
			if(!is_a_bus_req){
				...
				DMSG_UOC("<A_HOST> enter a_suspend state\n");
				break;
			}
            //printk("<A_HOST>:0x%x,id=%d,abusreq=0x%x,stat=%d\n",is_b_conn,get_otg_id(),is_a_bus_req,get_otg_state());
			uoc->transceiver.state = OTG_STATE_A_WAIT_BCON;/*manual change*/
		#ifdef CONFIG_USB_AM7X_HOST_ONLY
		    DMSG_UOC("<A_HOST>leave A-HOST state\n");
			if(uoc->transceiver.host){
...
#if 1 	
	DMSG_UOC("<B_IDLE> state transfer from %02x ------>%02x\n",
			aotg->prevstate,aotg->transceiver.state);
	DMSG_UOC("%s:prevstate%s\n",__FUNCTION__,state_string(aotg->prevstate));
#endif
	...
	switch(aotg->prevstate) {
		...
		case OTG_STATE_B_PERIPHERAL:
			...
			if(aotg->transceiver.gadget){
				...
				DMSG_UOC("<B_IDLE>no notify handler\n");
			}
			break;
	#if 0		
		...
		case OTG_STATE_A_WAIT_BCON:
			if(aotg->transceiver.gadget){
				DMSG_UOC("<B_IDLE>ID SWITCH\n");	
				enable_udc_interrupt();
				//otg_peripheral_notify(AOTG_SOFT_DISCONN_EVENT);
			}
	#endif		
		default:
			DMSG_UOC("<B_IDLE>odd state\n");
			break;
	}
	update_state(aotg);
}
...
static void aotg_id_switch(struct aotg_uoc * aotg,int newid)
{
	...
	if(aotg->transceiver.default_a) {
		...
		DMSG_UOC("<ID_SWITCH>change to B device\n");
		switch(aotg->prevstate){
			case OTG_STATE_A_HOST:
			...
			break;
			default:
				DMSG_UOC("<A_HOST>middle state not deal\n");
			break;
		}
		...
		switch(aotg->transceiver.state) {
			case OTG_STATE_B_IDLE:
				DMSG_UOC("<ID_SWITCH>change to A device\n");
				aotg_clearbits(OTGCTRL_ABUSDROP,OTGCTRL);
				break;
#if 0 				
			case OTG_STATE_B_DISCHRG1:
			case OTG_STATE_B_SRP_INIT1:
				DMSG_UOC("<ID_SWITCH> quit SRP,and change to a device\n");
				/*TODO: notify SRP stopping to userspace*/
				break;
#endif				
			case OTG_STATE_B_PERIPHERAL:
				DMSG_UOC("<ID_SWITCH>stop b_peripheral activity, and change to a device\n");
				if(aotg->transceiver.gadget){
					//otg_peripheral_notify(AOTG_ID_CHANGE_EVENT);
				}else {
					aotg_clearbits(OTGCTRL_ABUSDROP,OTGCTRL);
					DMSG_UOC("<ID_SWITCH>no notify handler\n");
				}
				break;
				
			default:
				DMSG_UOC("<ID_SWITCH>odd state %02x\n",aotg->transceiver.state);
				break;
		}
	} 
...
static void aotg_b_peripheral(struct aotg_uoc * aotg)
{
	...
	switch(aotg->prevstate) {
		case OTG_STATE_B_IDLE:
			...
			break;		
		case OTG_STATE_B_PERIPHERAL:
			DMSG_UOC("<B_PERIPHERAL> already in b_peri state,do nothing\n");
			break;
#if 0 
			if(aotg->transceiver.gadget){
				...
				DMSG_UOC("<B_PERIPHERAL>no notify handler\n");
			break;
#endif		
		...
		case OTG_STATE_A_HOST:
		case OTG_STATE_A_WAIT_BCON:
			DMSG_UOC("###***<B_PERIPHERAL>id switch!!!\n");
			if(aotg->transceiver.gadget)	
				enable_udc_interrupt();
			break;	
#endif		
		...
		default:
			DMSG_UOC("<B_PERIPHERAL> odd state\n");
			break;
	}
	update_state(aotg);
}
	...
	switch(aotg->prevstate) {
		...
		case OTG_STATE_A_SUSPEND:
			if(aotg->transceiver.host){
				...
				DMSG_UOC("<A_BUS_ERR>no notify handler\n");
			break;
		default:	
			break;
	}										 	
...
static inline void port_event_inform(struct aotg_uoc * uoc,int portevent)
{
	...
	if(!phandler){
		DMSG_OTG("not event inform handler\n");	
		return;
	}
	DMSG_OTG("%s:portevent=0x%x\n",__FUNCTION__,portevent);
	phandler->function((void*)phandler->data,portevent);
}


int otg_set_porthandler_next(struct otg_transceiver *transceiver,struct port_handler *process)
{
	...
	DMSG_UOC("Set port handler sucess!\n");
	return 0;
}
EXPORT_SYMBOL(otg_set_porthandler_next);
		...
		switch(otgint) {
			...
			case AOTG_IRQ_HOST_AND_IDLE_AND_PERI:
				...
				if( id == uoc->id){ 	
					...
					/*id switch*/
					DMSG_UOC("id changed,id= %d,orig=%d\n",id,uoc->id);
					aotg_id_switch(uoc,id);
				}
				break;
			case AOTG_IRQ_PERIPH:
				//printk("[%s] aotg periph irq!\n",__FILE__);
...
#endif
	//INFO("estatus:%x,idle:%x\n",estatus,is_bidle);
	if(/*!(estatus&(1<<6))&&*/((estatus & AOTG_DEVIN_MASK) != 0) &&  is_bidle){	
		result = 1;
		//aotg_writeb(estatus&0x0f,USBESTA);
	}
	return result;
}
...
int get_cur_usb_inout_status_next(void)
{
	return usb_cur_inout_status_next;
}
EXPORT_SYMBOL(get_cur_usb_inout_status_next);
 
// T166,William 20150814: set current USB plug in/out status
void set_cur_usb_inout_status_next(int status)
{
	usb_cur_inout_status_next = status;
}
EXPORT_SYMBOL(set_cur_usb_inout_status_next);
#endif
...
static void plug_detect_handler(unsigned long param)
{
	...
	if(is_dev_plugin(uoc)){
		...
		INFO("<HOST> current status: %d\n", usb_cur_inout_status_next);
		if (usb_cur_inout_status_next == USB_CUR_STATUS_PLUG_OUT)
		{
			INFO("<HOST>plug in\n");
			msg.type = SYSMSG_USB;
			msg.subtype = HOST_USB_IN;
			msg.dataload = HOST_RAW_NEXT;
			msg.reserved[0]=USB_GROUP1;
			am_put_sysmsg(msg);
			enable_plug_check(uoc,0);
			usb_cur_inout_status_next = USB_CUR_STATUS_PLUG_IN;
		}
		else
		{
			INFO("<HOST>Ignore plug in event because status is plug in now\n");
		}
#else
                INFO("<HOST>plug in\n");
                msg.type = SYSMSG_USB;
	...
	*/
	...
	if(vbus_state != uoc->vbus_gpio.val){
		...
		if(!vbus_state){ //inform udc for discon
			INFO( "[am7x_uoc_next] <DEV>plug out\n");
			...
			if(dtype==TYPE_PC_CONNECT){
				if(uoc->transceiver.gadget)
					...
					msg.dataload = DEVICE_MASS_STORAGE_NEXT;
					msg.reserved[0]=USB_GROUP1;
					am_put_sysmsg(msg);	
				}
			...
			INFO("[am7x_uoc_next]<DEV>plug in\n");
...
{	
	...
	mdelay(5);
#ifdef CONFIG_AM_8251
	aotg_writeb(aotg_readb(USBEXTCTRL) | 0x10,USBEXTCTRL);
#endif
	success = (get_otg_id() == value);	
	if(!success){	
		ERR("force ID->%c failed!!\n",role);
		spin_unlock_irqrestore(&uoc->lock,flags);
		return -EFAULT;
	}	
	...
	if(!host){  
		...
		*/
		...
		update_state(aotg);
			...
			INFO("eye figure test: portctl=0x%x\n",aotg_readb(HCPORTCTRL));
			while(1);
	#endif
		/*	
...
#endif
	INFO(" <am7x_uoc_next %s>otgstatus=%x,otgstata=%x,otgctrl=%x,usbesta=%x\n",__FUNCTION__,
...
static int aotg_set_peripheral(struct otg_transceiver *transceiver, struct usb_gadget *gadget) 
{
	...
	INFO("<am7x_uoc_next %s>otgstatus=%x,otgstate=%x usbesta=%x\n",__FUNCTION__,
		aotg_readb(OTGSTATUS),aotg_readb(OTGSTATE),aotg_readb(USBESTA));
	DMSG_UDC("<AOTG_SET_PERIPH>set periph success!\n");
	return 0;
}
...
static int aotg_set_power(struct otg_transceiver *transceiver,int is_on)
{
	...
	struct aotg_uoc  	* aotg = transceiver_to_aotg(transceiver);
	DMSG_UOC("<TRANSCEVIER_POWER> set power :%d\n",is_on);
	...
	if(is_on){
		if(!is_dev_plugin(aotg)){
			dump_otg_regs();
			enable_plug_check(aotg,1);
			DMSG_UOC("<TRANSCEVIER_POWER>dev non exist\n");
			return -ESHUTDOWN;
		}
		
		if(state == OTG_STATE_A_IDLE){			
			DMSG_UOC("<TRANSCEVIER_POWER> power on\n");
			aotg->transceiver.state = OTG_STATE_A_WAIT_BCON;
			aotg_clearbits(OTGCTRL_ABUSDROP,OTGCTRL); 
			aotg_setbits(OTGCTRL_BUSREQ,OTGCTRL); 
		}else if ((state == OTG_STATE_A_HOST ) ||(state == OTG_STATE_A_WAIT_BCON)){
			DMSG_UOC("<TRANSCEVIER_POWER>aleady power on,do nothing\n");	
		}else{
			ERR("<TRANSCEVIER_POWER>inappropriate OTG state,hw: %02x  sw: %02x\n",
					state,aotg->transceiver.state);
			dump_otg_regs();
			return -EFAULT;
		}			
	}else{
		DMSG_UOC("<TRANSCEVIER_POWER> power off\n");
		aotg_clearbits(OTGCTRL_BUSREQ,OTGCTRL);
...
struct otg_transceiver * otg_get_transceiver_next (void)
{		
       return aotg_to_transceiver (the_controller);
}
EXPORT_SYMBOL(otg_get_transceiver_next);

/*-----------------------------------------------------------*/
static void set_vbus_detect_threshold(unsigned int th)
{
	unsigned long  val;
	val = aotg_readl(USB2_PHY_CTL); //1213 page 473
	DMSG_UOC("%s:old val:0x%lx\n",__FUNCTION__,val);
	val &=0xffffffcf;
	val |= (th << 4);
	aotg_writel(val, USB2_PHY_CTL);
	DMSG_UOC("%s:new val:0x%08x\n",__FUNCTION__,aotg_readl(USB2_PHY_CTL));
}
...
static int usb_reset(void)
{
	...
	for(cnt=0;cnt < USB_RESET_TICKS;cnt++)
	{
		...
		/*if the product is ezcast,wo should disable disconnct detect*/
			...
			//uoc_set_phy(0xe7,0x1f); 
			#else
			printk("EZCast-1 disable disconn detect\n");
			uoc_set_phy(0xe7,0x0f); 
			#endif
		#else
			uoc_set_phy(0xe7,0x1f);  
	...
	DMSG_UOC("<USB_RESET>cannot reset usb otg\n");
	return -EBUSY;
}
...
static int aotg_uoc_enable(struct aotg_uoc * uoc,int enable)
{
	...
	if(enable){
		...
		if(0 == internal_resistor){
			...
			if(!(efuse_val & (1<<6))){
				INFO("use efuse val from CHIP ID\n");
				...
				INFO("use efuse val from efuse reg\n");
				INFO("orignal val:%x\n", efuse_val & 0x3f);
			}
		...
		#endif
		uoc_set_phy(0xe7,0x1f);//enable disconnect check
...
{	
	...
	DMSG_UOC( "<aotg_uoc_remove> enter remove\n");

#ifdef UOC_FILE_OPERATION
	device_destroy(uoc->uoc_next_class,uoc->uoc_next_char);
	 unregister_chrdev_region(uoc->uoc_next_char, 2);
	 class_destroy(uoc->uoc_next_class);
#endif
...
static int __init aotg_uoc_init(void)
{
	...
	tmp = get_sys_info()->sys_gpio_cfg.usb_det1;
#else
	...
	if((retval = usb_clk_open())!=0){
		INFO("clk open failed\n");
        goto handle_err;
	}

	/*request irq*/
	if(request_irq(IRQ_USB2,
				aotg_uoc_irq,
				IRQF_SHARED |IRQF_DISABLED,
				driver_name,
				(void*)uoc) != 0 ) {
		ERR (" uoc request irq %d failed\n",IRQ_USB2);
		retval = -EBUSY;
		goto handle_err;
	}
	if(aotg_uoc_enable(uoc,1) < 0){
		ERR (" uoc enable failed\n");
		retval = -EFAULT;
		goto handle_err1;
	}		
#ifdef UOC_FILE_OPERATION  //register char driver
	int result;
	uoc->uoc_next_class= class_create(THIS_MODULE,"usb-uoc-next");
	if (IS_ERR(uoc->uoc_next_class)) {
		result = PTR_ERR(uoc->uoc_next_class);
		ERR("unable to create usb_gadget class %d\n", result);
		//goto failed;
	}

	if ( (result = alloc_chrdev_region(&uoc->uoc_next_char, 
			0, 1,"usb_uoc_next")) < 0) {
		ERR("alloc_chrdev_region %d\n", result);
		//goto failed1;
	}

	device_create_drvdata(uoc->uoc_next_class, NULL,
				      uoc->uoc_next_char,
				      NULL, "usb_uoc_next");

	cdev_init(&uoc_next_dev, &uocnext_operations);
	uoc_next_dev.owner = THIS_MODULE;
	result= cdev_add(&uoc_next_dev, uoc->uoc_next_char, 1);
	if (result) {
	...
	if(!uoc->is_otg){
		...
		if(retval != 0){
			ERR ("create sys entry a_plug failed!!\n");
			device_remove_file(&uoc->dev, &dev_attr_b_type);
			device_remove_file(&uoc->dev, &dev_attr_b_plug);
			goto handle_err2;
		}	
	...
	full_uoc=uoc;
#endif
	INFO("%s: probe success!\n",driver_name);
	return 0;
	
handle_err2:
	device_unregister(&uoc->dev);
handle_err1:
	free_irq(IRQ_USB2,(void *)uoc);
handle_err:
	kfree(uoc);
