...
static const char driver_name [] = "aotg-usb-am7x-";
...
static void proc_ep_show(struct seq_file *s, struct aotg_ep *ep)
{
	...
	if(!strcmp(ep->ep.name,ep0name)){
		...
		seq_printf(s,"\t<am7x_udc>@@@--- ep0state:%s\n",ep0_string[udc->ep0state]);
		return;
	}
	
	spin_lock_irqsave(&udc->lock,flags);
	tmp = aotg_readb(ep->reg_udccon);
	seq_printf(s,"\t<am7x_udc>@@@--- udccon(%lx):%02x\n",ep->reg_udccon,tmp);
	seq_printf(s, "\t<am7x_udc>@@@--- buf_conf:%s,ep_type:%s,stall:%d,valid:%d\n",
			buf[tmp&0x03],type[(tmp>>2)&0x03],
			(tmp>>6)&0x01,(tmp>>7)&0x01);
	
	seq_printf(s,"\t<am7x_udc>@@@--- Regcs(0x%lx):0x%x,FifoAddr(0x%lx): 0x%x,udcbc(0x%lx):%x\n",
	...
	if (list_empty (&ep->queue))
		seq_printf(s, "\t<am7x_udc>@@@--- (queue empty)\n");
	else{
		list_for_each_entry (req, &ep->queue, queue) {		
			seq_printf(s, "\t<am7x_udc>@@@--- req %p len %d/%d buf %p\n",
				&req->req,
				req->req.actual,
				req->req.length, 
				req->req.buf);
...
static void proc_dma_show(struct seq_file *s)
{
	...
	seq_printf(s,"\n<am7x_udc>@@@--- dma_channel:%d\n",udc->dma_channel);
	seq_printf(s,"\t<am7x_udc>@@@---  dma irq:%d\n",am_get_dma_irq(udc->dma_channel,1,0));
}
...
static int proc_udc_show(struct seq_file *s, void *unused)
{
	struct aotg_udc	*udc = s->private;
	struct aotg_ep	*ep;
	char *state_string[] = {"<am7x_udc>@@@--- unknown","idle","active","suspend","disable"};

	seq_printf(s, "\n<am7x_udc>@@@--- %s: version %s\n", driver_name, DRIVER_VERSION);
	seq_printf(s, "\n<am7x_udc>@@@--- gadget:%s\n", udc->driver ? udc->driver->driver.name : "(none)");
	seq_printf(s, "\n<am7x_udc>@@@--- udcstate:%s\n",state_string[udc->state]);
	
	/* don't access registers when interface isn't clocked */
	seq_printf(s, "\n<am7x_udc>@@@--- FSM: 0x%02x,OTGSTATUS:0x%02x USBESTA:0x%02x\n", 
		aotg_readb(OTGSTATE) & 0x0f,aotg_readb(OTGSTATUS),
		aotg_readb(USBESTA));

	seq_printf(s, "\n<am7x_udc>@@@--- OTGIEN: 0x%02x: USBIEN:0x%02x\n",
...
static void dump_ep_regs(struct aotg_ep *ep)
{
	...
	DMSG_UDC("<am7x_udc>@@@--- %s:\n",ep->ep.name);
	DMSG_UDC("<am7x_udc>@@@--- epcon=0x%x\n",aotg_readb(ep->reg_udccon));
	DMSG_UDC("<am7x_udc>@@@--- epcs=0x%x\n",aotg_readb(ep->reg_udccs));
	DMSG_UDC("<am7x_udc>@@@--- epbc=0x%x\n",aotg_readw(ep->reg_udcbc));
	DMSG_UDC("<am7x_udc>@@@--- IE=0x%x\n",aotg_readb(ien));
	DMSG_UDC("<am7x_udc>@@@--- IRQ=0x%x\n",aotg_readb(irq));	
}
#else
...
static void ep_irq_disable(u8 mask)
{
	...
	DMSG_XFERS("<am7x_udc>@@@--- <PIO_IRQ_DISABLE> ep mask %02x pio irq disable\n",mask);
	if(is_in)
...
static void ep_irq_enable(u8 mask)
{
	...
	DMSG_XFERS("<am7x_udc>@@@--- <PIO_IRQ_ENABLE>ep mask %02x pio irq enable\n",mask);
	if(is_in)
...
static void ep_irq_clear(u8 mask)
{
	...
	DMSG_XFERS("<am7x_udc>@@@---  <PIO_IRQ_CLEAR>ep mask %02x pio irq clear\n",mask);
	if(is_in)
...
static void done(struct aotg_ep *ep, struct aotg_request *req, int status)
{
	...
	if (status && status != -ESHUTDOWN)
		DMSG_XFERS("<am7x_udc>@@@--- <REQ RELEASE>complete %s req %p stat %d len %u/%u\n",
...
static void nuke(struct aotg_ep *ep, int status)
{
	...
	DMSG_XFERS("<am7x_udc>@@@--- <EP NUKE> %s is nuked with status  %d\n",ep->ep.name,status);  
	if(use_dma && dma_channel > 0){
	 	if(ep->dma_working && ! ep->stopped){
			INFO("<am7x_udc>@@@--- cancel ep used dma\n");
...
static int read_ep0_fifo (struct aotg_ep *ep, struct aotg_request *req)
{
	...
	DMSG_CTRL("<am7x_udc>@@@---ep0out %d bytes %s %d left %p\n", length,
...
static int write_ep0_fifo (struct aotg_ep *ep, struct aotg_request *req)
{
	...
	DMSG_CTRL("<am7x_udc>@@@---ep0in %d bytes %s %d left %p\n", count,
...
static inline void handle_status(void)
{
	...
	DMSG_CTRL("<am7x_udc>@@@---<CTRL>ACK the status stage\n");
}
...
#define w_length		le16_to_cpu(u.r.wLength)
	...
	DMSG_CTRL( "<am7x_udc>@@@---<CTRL> SETUP %02x.%02x v%04x i%04x l%04x\n",
	...
	switch (u.r.bRequest) {
		case USB_REQ_GET_STATUS:
			DMSG_CTRL("<am7x_udc>@@@---<CTRL>USB_REQ_GET_STATUS\n");
			...
			*/
				...
				DMSG_CTRL("<am7x_udc>@@@---GET STATUS,name:%s:ep addr:%x\n",
		...
		case USB_REQ_CLEAR_FEATURE:
			DMSG_CTRL("<am7x_udc>@@@---<CTRL> USB_REQ_CLEAR_FEATURE\n");
			if((u.r.bRequestType == USB_RECIP_DEVICE) && (u.r.wValue == USB_DEVICE_REMOTE_WAKEUP)){
				DMSG_CTRL("<am7x_udc>@@@---<CTRL> clear remote wakeup feature\n");
				...
				DMSG_CTRL("<am7x_udc>@@@---<CTRL> clear %s halt feature\n",ep->ep.name);
			}else 
				goto stall;
		...
		case  USB_REQ_SET_FEATURE:
			DMSG_CTRL("<am7x_udc>@@@---<CTRL> USB_REQ_SET_FEATURE\n");
...
delegate:			
	DMSG_CTRL("<am7x_udc>@@@---<CTRL>delegate\n");
	...
	if(status < 0) {
stall:
		DMSG_CTRL("<am7x_udc>@@@---<CTRL> req %02x.%02x protocol STALL, err  %d\n",
			u.r.bRequestType, u.r.bRequest, status);
		if(udc->req_config)
			DMSG_UDC("<am7x_udc>@@@---<CTRL>config change erro\n");
		aotg_setbits(EP0CS_STALL,EP0CS);
		udc->req_pending = 0;
		udc->ep0state = EP0_STALL;
	}
...
static void handle_ep0(struct aotg_udc *_udc)
{
	...
	DMSG_CTRL("<am7x_udc>@@@---<CTRL>ep0 irq handler,state is %d,queue is %s\n",
	...
	switch (udc->ep0state) {
		...
		case EP0_OUT_DATA_PHASE:
			if(req) {
				...
				DMSG_CTRL("<am7x_udc>@@@---<CTRL>ep0out irq error,queue is empty but state is EP0_OUT_DATA_PHASE\n");/*never enter this branch*/
			break;
		...
		default:
			DMSG_CTRL("<am7x_udc>@@@---<CTRL>ep0in irq error,odd state %d\n",udc->ep0state);
...
static int write_fifo(struct aotg_ep *ep, struct aotg_request *req)
{
	...
	while (!(aotg_readb(ep->reg_udccs) & EPCS_BUSY)){
		...
		if (is_last) {
			...
			if (list_empty(&ep->queue)){
				DMSG_BULK("<am7x_udc>@@@---%s queue is empty now,do nothing\n",ep->ep.name);
				ep_irq_disable (ep->mask);		
			}
			DMSG_BULK("<am7x_udc>@@@---data transferred? reg_udccs %02x\n",aotg_readb(ep->reg_udccs));
			return 1;
		}
	}
...
static int read_fifo(struct aotg_ep *ep, struct aotg_request *req)
{
	...
	int i;
		...
		DMSG_BULK("<am7x_udc>@@@---%s read  %d bytes%s %s actual %d bytes claim %d/bytes\n",
...
static void handle_ep(struct aotg_udc * dev, u8 index)
{
	...
	do {
		...
		if (!list_empty(&ep->queue))
			...
			DMSG_BULK("<am7x_udc>@@@---<BULK>%s queue is empty,do nothing\n",
				ep->ep.name);
			req = NULL;
			return;
		}
		...
		if(is_in)
			...
			if(!(aotg_readb(ep->reg_udccs) & EPCS_BUSY)){
				DMSG_BULK("<am7x_udc>@@@---<BULK>%s queue and fifo are all not empty,reading, bytes %d\n",ep->ep.name,req->req.length);
		...
		const struct usb_endpoint_descriptor *desc)
{
	...
	if (!_ep || !desc || ep->desc || _ep->name == ep0name
		...
		ERR("<am7x_udc>@@@---%s, bad ep or descriptor\n", _ep->name);
		return -EINVAL;
	}

	/*not support ISO yet*/
	if (desc->bmAttributes == USB_ENDPOINT_XFER_ISOC) {
		ERR("<am7x_udc>@@@---%s, ISO nyet\n", _ep->name);
		return -EDOM;
	}
	...
	if (ep->bmAttributes != desc->bmAttributes
		...
		ERR("<am7x_udc>@@@---%s: %s type mismatch\n", __FUNCTION__, _ep->name);
		return -EINVAL;
	}
	...
	if ((desc->bmAttributes == USB_ENDPOINT_XFER_BULK
		...
		|| !desc->wMaxPacketSize) {
		ERR("<am7x_udc>@@@---%s: bad %s maxpacket\n", __FUNCTION__, _ep->name);
		return -ERANGE;
	}

	udc = ep->dev;
	if (!udc->driver || udc->gadget.speed == USB_SPEED_UNKNOWN) {
		ERR("<am7x_udc>@@@---%s: bogus device state\n", __FUNCTION__);		
		return -ESHUTDOWN;
	}
	...
	DMSG_UDC("<am7x_udc>@@@---<EP ENABLE>%s enable,reg_udccon is %02x\n",
...
static int aotg_ep_disable (struct usb_ep *_ep)
{
	...
	ep = container_of(_ep,struct aotg_ep,ep);
	if (!_ep || !ep->desc) {
		DMSG_UDC("<am7x_udc>@@@---<EP DISABLE> %s not enabled\n",
			_ep ? ep->ep.name : NULL);
		return -EINVAL;
	}
	udc = ep->dev;
	...
	DMSG_UDC("<am7x_udc>@@@---<EP DISABLE>%s disable\n",_ep->name);
	return 0;
}
...
aotg_ep_queue(struct usb_ep *_ep, struct usb_request *_req,gfp_t gfp_flags)
{
	...
	if (!_req || !_req->complete || !_req->buf
		...
		DMSG_XFERS("<am7x_udc>@@@---<EP QUEUE>bad params\n");
		return -EINVAL;
	}

	ep = container_of(_ep, struct aotg_ep, ep);
	if (!_ep || (!ep->desc && ep->ep.name != ep0name)) {
		DMSG_XFERS("<am7x_udc>@@@---<EP QUEUE> bad ep\n");
		return -EINVAL;
	}

	udc = ep->dev;
	if (!udc->driver || udc->gadget.speed == USB_SPEED_UNKNOWN) {
		DMSG_XFERS("<am7x_udc>@@@---<EP QUEUE> bogus device state\n");
		return -ESHUTDOWN;
	}
	...
	if(is_csw(req)){
		DMSG_XFERS("<am7x_udc>@@@---<EP QUEUE>%s queue req %p, len %d buf %p,ep = %p,stopped = %d\n",
	...
	*/
	...
	if(list_empty(&ep->queue) && ! ep->stopped) {
		...
		if(ep->bEndpointAddress == 0) {
			/*ep0*/
			if(!udc->req_pending) {
				DMSG_XFERS("<am7x_udc>@@@---<EP QUEUE> something wrong with Control Xfers,req_pending is missing\n");				
				retval =  -EL2HLT;
				goto endl;
			}
			...
			case EP0_OUT_DATA_PHASE:
				...
				if(!req->req.length) {
					/*ACK*/
					DMSG_CTRL("<am7x_udc>@@@---no data ,enter status stage\n");
					handle_status();
					/*cleanup*/
					udc->req_pending = 0;
					udc->ep0state = EP0_WAIT_FOR_SETUP;
			...
			case EP0_IN_DATA_PHASE: 
				...
				break;
			default:
				DMSG_XFERS("<am7x_udc>@@@---<EP QUEUE> ep0 i/o, odd state %d\n",udc->ep0state);
				retval =  -EL2HLT;
				goto endl;
			}
		}
			...
			if(aotg_udc_kick_dma(ep , req) == 0){
				...
				if((req->req.length != 0) && (req->req.length == req->req.actual)){
					DMSG_DMA("<am7x_udc>@@@---req.length:%x ,req.actual:%x req :%x \n",
			...
			if ((ep->bEndpointAddress & USB_DIR_IN) != 0){
				if(!(aotg_readb(ep->reg_udccs) & EPCS_BUSY)) {/*fifo can access*/
					DMSG_XFERS("<am7x_udc>@@@---<EP QUEUE>%s fifo is all ready,wrtie immediately\n",ep->ep.name);
				...
				if(!(aotg_readb(ep->reg_udccs) & EPCS_BUSY)) {
					...
					DMSG_XFERS("<am7x_udc>@@@---<EP QUEUE>%s fifo has data already,read immediately\n",ep->ep.name);
					ep_irq_clear(ep->mask);/*avoid nonsensical interrupt*/
					if(read_fifo(ep,req))
						req = NULL;
				}
			...
			if( (ep->dma_working == 0)&&(req == NULL) && (ep->bmAttributes ==  USB_ENDPOINT_XFER_INT) && !list_empty(&ep->queue) ){
				ep_irq_enable(ep->mask);
				if( ep->bmAttributes ==  USB_ENDPOINT_XFER_INT){
				      printk("-------- enable  int ep interrupt2\n");
				}
			}
		}
	}
	...
	if (req != NULL){
		...
		DMSG_UDC("<am7x_udc>@@@---<EP QUEUE>%s:req not done completely,queueing and wait\n",ep->ep.name);
	}
	retval = 0;
endl:	
	local_irq_restore(flags);
...
static int aotg_ep_dequeue(struct usb_ep *_ep, struct usb_request *_req)
{
	...
	DMSG_XFERS("<am7x_udc>@@@---<EP DEQUEUE> %s dequeues one req %p\n",_ep->name,_req);
	ep = container_of(_ep,struct aotg_ep,ep);
	udc = ep->dev;
...
static int aotg_ep_set_halt(struct usb_ep *_ep, int value)
{
	...
	ep = container_of(_ep,struct aotg_ep,ep);
	if (!_ep || (!ep->desc && ep->ep.name != ep0name)) {
		DMSG_UDC("<am7x_udc>@@@---<EP HALT>, bad ep\n");
		return -EINVAL;
	}	
...
static void  aotg_ep_fifo_flush(struct usb_ep* _ep)
{
	...
	if (!_ep || (!ep->desc && ep->ep.name != ep0name)) {
		DMSG_UDC("<am7x_udc>@@@---<FIFO FLUSH>bad ep\n");
		return;
	}	
...
static int aotg_udc_get_frame(struct usb_gadget *_gadget)
{
	...
	DMSG_UDC("<am7x_udc>@@@---<UDC_GET_FRAME>Frame No.: %d\n", frmnum);
	return frmnum;
...
static int aotg_udc_wakeup(struct usb_gadget *_gadget)
{
	...
	if(udc->rwk) {
        ...
        DMSG_UDC("<am7x_udc>@@@---<UDC_WAKEUP>host  %s support \n",udc->rwk?"can" : "can't");
	return retval;
}
...
static int aotg_udc_vbus_session(struct usb_gadget *_gadget, int is_active)
{
	...
	DMSG_UDC("<am7x_udc>@@@---<UDC_VBUS_SESSION> VBUS %s\n", is_active ? "on" : "off");
	udc = container_of(_gadget,struct aotg_udc,gadget);
...
{  
	...
	if(isenable){
		...
		DMSG_UDC("<am7x_udc>@@@---<UDC_ENABLE> %p,AOTG enters :%d state\n",dev,aotg_readb(OTGSTATE));
		dplus_up();
		mb();
	}
	else{
		DMSG_UDC("<am7x_udc>@@@---<UDC_DISABLE> %p\n", dev);
...
int usb_gadget_register_driver(struct usb_gadget_driver *driver)
{
	...
	DMSG_UDC("<am7x_udc>@@@---gadget register\n");
	if (!driver
	...
	if (udc->driver) {
		ERR("<am7x_udc>@@@---one gadget drvier has already registered\n");
		retval = -EBUSY;
		return retval;
	}
	...
	if (retval) {
		ERR("<am7x_udc>@@@---%s: bind to otg driver %s  error %d\n", udc->gadget.name,
	...
	*/
	...
	retval = otg_set_porthandler(udc->transceiver,process);
	if(retval < 0 ){
		ERR("<am7x_udc>@@@---<UDC_PROBE>otg set porthandler failed\n");
		otg_set_peripheral(udc->transceiver,NULL);
		return retval;
	}
	...
	if (retval) {
		ERR("<am7x_udc>@@@---%s: bind to driver %s  error %d\n", udc->gadget.name,
	...
	pullup(udc,1);
	DMSG_UDC("<am7x_udc>@@@---register ok,pull up to let pc see us\n");
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	/*pull down*/
	pullup(udc, 0);	
	/*add this to fix crash issue when remod g_ether. denny*/
	...
	retval = otg_set_porthandler(udc->transceiver,NULL);
	if(retval < 0){
		ERR("<am7x_udc>@@@---unset porthandler failed\n");
		return retval;
	}

	retval = otg_set_peripheral(udc->transceiver,NULL);	
	if(retval){
		ERR("<am7x_udc>@@@---set peripheral failed\n");
		return retval;
	}
	...
	DMSG_UDC("<am7x_udc>@@@---%s: unregistered gadget driver '%s'\n", 
...
static irqreturn_t aotg_udc_irq(int irqnum,void *_udc)
{
	...
	if(irqvector == UIV_OTGIRQ){
		DMSG_UDC("<am7x_udc>@@@---the irq not belongs to uoc\n");
		return IRQ_HANDLED;
	}

	DMSG_UDC("<am7x_udc>@@@---irqvector=%x----\n",irqvector);
	switch(irqvector){			
		case UIV_USBRESET:
			...
			DMSG_UDC("<am7x_udc>@@@---gadget %s,USB reset done\n",udc->driver->driver.name);
...
static void aotg_udc_start_dma(struct aotg_ep * ep, struct aotg_request * req , int is_in)
{
	...
	DMSG_DMA("<am7x_udc>@@@---dma:%x->%x len:%d\n", buf_addr,fifo_addr,dcmd);
	...
	DMSG_DMA("<am7x_udc>@@@---<start_dma>:arm end........\n");
}
...
static void aotg_udc_cancel_dma(struct aotg_ep * ep)
{
	//struct aotg_udc * udc = ep->dev;	
	u32	dma_channel;
	DMSG_DMA("<am7x_udc>@@@---<UDC_CANCEL_DMA>\n");
	dma_channel = ep->dma_nr;
	ep->dma_working = 0;
...
static unsigned int  aotg_udc_kick_dma(struct aotg_ep * ep, struct aotg_request * req)
{
	...
	DMSG_DMA("<am7x_udc>@@@---<kick_dma>length:%d,actual:%d\n",req->req.length,req->req.actual);	
	
	if(ep->bEndpointAddress & USB_DIR_IN){
		if((remains == 0) || (remains < ep->maxpacket)){
			DMSG_DMA("<am7x_udc>@@@---<IN>little pkt\n");
			ep_irq_enable(ep->mask);
			if(!(aotg_readb(ep->reg_udccs) & EPCS_BUSY)){
				return write_fifo(ep,req);
			}else
				DMSG_DMA("<am7x_udc>@@@--- otg ep busy now\n");		
		}else{
			DMSG_DMA("<am7x_udc>@@@---<kick_dma>:EP IN DMA,remains:%d\n",remains);
			ep->dma_working = 1;
			ep_irq_disable(ep->mask);
		...
		if((remains == 0) ||(remains <= ep->maxpacket)){
			DMSG_DMA("<am7x_udc>@@@--- <OUT>little pkt\n");
			ep_irq_enable(ep->mask);
			if(!(aotg_readb(ep->reg_udccs) & EPCS_BUSY))
				return read_fifo(ep , req);
		}else{
			DMSG_DMA("<am7x_udc>@@@---<kick_dma>:OUT: DMA trans\n");
			ep->dma_working = 1;
			ep_irq_disable (ep->mask);
...
static irqreturn_t aotg_udc_dma_handler(int irqnum,void *_udc)
{
	...
	if(!am_get_dma_irq(udc->dma_channel,1,0)){
		...
		if(!am_get_dma_irq(udc->dma_channel2,1,0)){	
			DMSG_DMA( "<am7x_udc>@@@---%s:irq may not belongs to us!\n",__FUNCTION__);			
			return IRQ_NONE;
		}
		else
	...
	if(!ep){
		ERR("<am7x_udc>@@@---odd ,no ep dma is working!\n");
		return IRQ_HANDLED;
	}
	...
	DMSG_DMA("<am7x_udc>@@@---remains:%d,length:%d,actual:%d,maxpacket:%d\n",
	...
	if(ep->bEndpointAddress & USB_DIR_IN){
		if( ((remains > 0) &&( remains < ep->maxpacket) ) ||
			...
			if(aotg_readb(ep->reg_udccs) & EPCS_BUSY){
				DMSG_DMA("<am7x_udc>@@@---<dma_handler>:IN BUSY..\n");
				ep_irq_enable(ep->mask);	
				ep->dma_working = 0;
			}else{
				DMSG_DMA("<am7x_udc>@@@---<dma_handler>:IN NOT busy..\n");
			...
			DMSG_DMA("<am7x_udc>@@@---<dma_handler>:remains:%d\n",remains);
			aotg_udc_kick_dma(ep , req);	
		}else{ 
			//complete
			DMSG_DMA("<am7x_udc>@@@---<dma_handler>: req ok\n");
			done(ep, req, 0);
			ep->dma_working = 0;
			completed = 1;
		}
		...
		if ( (remains > 0) && ( remains < ep->maxpacket) ){
			DMSG_DMA("<am7x_udc>@@@---<dma_hander>:OUT short packet\n");
			...
			DMSG_DMA("<am7x_udc>@@@---<dma_hander>:packet compeleted\n");	
			ep->dma_working = 0;
			done(ep ,req ,0);	
			completed = 1;
		}		
	...
	DMSG_DMA( "<am7x_udc>@@@---exit dma handler\n");
	return IRQ_HANDLED;
}
...
static void config_ep_fifo_addr(void)
{
	...
	DMSG_UDC("<am7x_udc>@@@---ep1out fifoaddr:%x\n",aotg_readl(OUT1STARTADDRESS));
	DMSG_UDC("<am7x_udc>@@@---ep1in fifoaddr:%x\n",aotg_readl(IN1STARTADDRESS));
	DMSG_UDC("<am7x_udc>@@@---ep2in fifoaddr:%x\n",aotg_readl(IN2STARTADDRESS));
}
 ...
 static int __init  am7x_udc_probe(struct platform_device *pdev)
{
	...
	if(!udc->transceiver) {
		ERR("<am7x_udc>@@@---<UDC_PROBE>no transceciver\n");
		retval = -ENODEV;
		goto end1;
	}
	...
	if(use_dma){
		retval = am_request_dma(DMA_CHAN_TYPE_SPECIAL,
				...
				0,(void *)udc);
		if(retval <0 ){
			ERR("<am7x_udc>@@@---<UDC_PROBE>request DMA failed\n");
			goto end1;
		}	
		...
		INFO("<am7x_udc>@@@---<UDC_PROBE>DMA request success ,chnl:%d\n",udc->dma_channel);
	...
	retval =  request_irq(IRQ_USB, aotg_udc_irq,
				...
				(void *)udc);
	if (retval < 0){
		ERR("<am7x_udc>@@@---<UDC_PROBE>request irq %d failed\n",IRQ_USB);
		goto end2;
	}
		//printk("debug---%s #####@@@@ :%d\n",__FUNCTION__,IRQ_USB);
	/*register driver*/
	retval = device_register(&udc->gadget.dev);
	if (retval < 0){
		ERR("<am7x_udc>@@@---<UDC_PROBE>register device driver failed!\n");
		goto end3;
	}
	
	create_debug_file(udc);
	dev_set_drvdata(dev,udc);
	INFO("<am7x_udc>@@@---%s:driver probe success\n",driver_name);
	return 0;
 ...
 static int __exit am7x_udc_remove(struct platform_device *pdev)
{
	...
	if(udc->driver){
		WARN("<am7x_udc>@@@---driver is still used\n");
		return -EBUSY;
	}
	...
	INFO("<am7x_udc>@@@---udc remove success\n");
	return 0;
}
