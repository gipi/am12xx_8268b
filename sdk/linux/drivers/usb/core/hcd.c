...
void usb_hcd_poll_rh_status(struct usb_hcd *hcd)
{
	...
	if (unlikely(!hcd->rh_registered)){
		printk("rh not registered\n");
		return;
	}
	
	//dev_info (hcd->self.controller,"%s:%d\n",__FUNCTION__,__LINE__);
...
static int usb_rh_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	if (usb_endpoint_num(&urb->ep->desc) == 0) {	/* Control URB */
		...
		if (!hcd->uses_new_polling){
			printk(KERN_INFO "del poll timer\n");
...
int usb_hcd_ioctl (struct usb_hcd * hcd, int cmd,int arg)
{
	if(!hcd->driver ||! hcd->driver->hcd_ioctl)
		return -EINVAL;
	
	return hcd->driver->hcd_ioctl(hcd,cmd,arg);
}
EXPORT_SYMBOL_GPL(usb_hcd_ioctl);
		...
		unsigned int irqnum, unsigned long irqflags)
{
	...
	if ((retval = register_root_hub(hcd)) != 0){
		printk(KERN_ALERT "register root hub failed\n");
	...
	dev_dbg(hcd->self.controller,"add hcd ok,begin to poll root hub status\n");
