...
static void uart_start(struct tty_struct *tty)
{
	...
	// this is because __uart_start(tty) move the data by cpu and occupied
	// too much time that locks the irq of uart to get data.
	// so don't save/restore irq, just lock the port for instead.
#if 0	
	spin_lock_irqsave(&port->lock, flags);
	__uart_start(tty);
	spin_unlock_irqrestore(&port->lock, flags);
