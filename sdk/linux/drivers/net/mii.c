...
void mii_dump_register(struct mii_if_info *mii){
	int i = MII_BMSR;
	printk("_______dump phy register______\n");
	for(;i <= MII_NCONFIG;i++)
		printk("id: 0x%02x\tval:0x%04x\n",i,mii->mdio_read(mii->dev, mii->phy_id, i));
...
EXPORT_SYMBOL(mii_dump_register);
