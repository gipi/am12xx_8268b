...
static void disconnect(struct usb_interface *intf)
{
	...
	ieee80211_free_hw(hw);
	#if 1
	struct usb_device *u_dev = interface_to_usbdev(intf);
	if(strncmp(u_dev->bus->bus_name,"aotg-usb-am7x-",strlen("aotg-usb-am7x-"))==0){
		usb_hotplug_msg(MSG_WLAN,0,1);	
	}else
		usb_hotplug_msg(MSG_WLAN,0,2);	
	#endif
