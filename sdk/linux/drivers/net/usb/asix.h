 ...
 * cannot handle non-double word aligned buffer. And also mask the function
 * "skb_reserve (skb, NET_IP_ALIGN)" in "usbnet.c".
 * When turn on this flag, driver will fixup egress packet aligned on double
 * word boundary before deliver to USB host controller. Disable the function
 * "skb_reserve (skb, NET_IP_ALIGN)" to retain the buffer aligned on double
 * word alignment for ingress packets.
 */
#define AX_FORCE_BUFF_ALIGN		0

/* 
 * If enabling checksum offload it is recommended to mask the function
 * "skb_reserve (skb, NET_IP_ALIGN)" in "usbnet.c". Without masking
 * this function, you will get unalignment issues of ip header on each
 * ingress packet.
 */
#define AX_772BHW_CHECKSUM		0
