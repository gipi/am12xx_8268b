...
static const char *socket_path = "/tmp/usbmuxd";
static const char *lockfile = "/tmp/usbmuxd.pid";
...
static int myppoll(struct pollfd *fds, nfds_t nfds, const struct timespec *timeout, const sigset_t *sigmask)
{
	int ready;
	sigset_t origmask;
	int to = timeout->tv_sec*1000 + timeout->tv_nsec/1000000;
...
static int main_loop(int listenfd)
{
	...
	while(!should_exit) {
		...
		cnt = myppoll(pollfds.fds, pollfds.count, &tspec, &empty_sigset);
