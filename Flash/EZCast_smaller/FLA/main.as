...
var EZCAST_LITE_ENABLE:Boolean = false;//(SystemInfo.getIncNumVal(SystemInfo.INC_EZCAST_LITE_ENABLE)==0)?false:true;	//true;
...
var isEZCastlite:Boolean = false;
...
var DEFAULT_MODE_SWF:String = EZCAST_SWF;
...
var mouse_in_flag:Boolean = false;
...
function loadResetPNG()
{
	trace("garen loadResetPng,wifi_display "+currentMC.wifi_display_on);
	if(currentMC.wifi_display_on){
		trace("wifi_display_off ");
		currentMC.wifiDisplayStop();
	}
	MiracastSWF.exitQRcode();
...
function key_midpress_check()
{
	trace("key_midpress_check!!");
	key_midpress_accept=true;
	if(timer_midcheck >= 0)
	{
		clearInterval(timer_midcheck);
		timer_midcheck = -1;
	}
	clearInterval(timer_midcheck);
}	
...
function load_defaultMode(){
	...
	if(EZCASTPRO_PROJECTOR && (EZCASTPRO_RUN == 0)){ // enter mainmenu
		loadswf("mainmenu.swf");
		TopBarSWF.toEZCastOff();
		trace("load_defaultMode:  enter mainmeu.swf");
		return;
	}
	...
	if(SWF_CHANGE_ENABLE){
		switch(last_ui){
			case LASTUI_3TO1 :
				...
				loadswf(STREAM_SWF);
				break;
			case LASTUI_EZMIRRLITE :
				if(EZCAST_LITE_ENABLE){
					isEZCastlite = true;
					connectMode = SystemInfo.ROUTER_ALLOW;
				}else{
					SystemInfo.set_lastui(LASTUI_EZMIRR, 0);
				}
			case LASTUI_EZMIRR :
			    if(EZCASTPRO_MODE==8075)
				{
					...
					MiracastDefault = true;
					swf_init();
					//MIRR_SWF = STREAM_SWF = DEFAULT_MODE_SWF = EZMIRR_SWF;
					trace("load miracast probox swf DEFAULT_MODE_SWF="+DEFAULT_MODE_SWF);
					trace("load miracast probox swf MIRR_SWF="+MIRR_SWF);
					loadswf(MIRR_SWF);
				}
				else
				{
			...
			default :
				loadDefaultUi();
				SystemInfo.set_lastui(DEFAULT_MODE_VAL, 0);
				break;
		}
	}else{
		loadDefaultUi();
		if(last_ui != DEFAULT_MODE_VAL)
			SystemInfo.set_lastui(DEFAULT_MODE_VAL, 0);
	}
}
...
{
	trace("load firstSWF@@@@@@@@@@");
	if((isEZCastpro()) && (!EZWILAN_ENABLE) && (EZCASTPRO_MODE!=8074)){
		if(SystemInfo.checkStartupUstatus())
		{
			trace("load connectPC.SWF")
			SystemInfo.acceptPCconInfo(1);
...
function is_ezcast_lite():Boolean {
	if(EZCAST_LITE_ENABLE && isEZCastlite)
		return true;
	else
		return false;
}
...
function swf_init(){
	reset_mc._visible =false;  //reset png not visible
	if(EZCASTPRO_MODE==8075&&SystemInfo.get_lastui()==2) 
	{
...
function showRouterInfoForP2pClient(){
	...
	if(lanstatus_priv != 0){
		...
		if(ipForLan != "" && ipForLan.indexOf(".", 2) > 0){
			TopBarSWF.showPrompt1("IP: "+ipForLan);
			prompt1Ok = true;
		}
	}
	if(EZCASTPRO_MODE==8075&&!prompt1Ok)//probox running here
	{
		if((WifiEngine.GetIndexofSelectAP() >= 0)||wifi_mode_get_from_case==WifiEngine.WIFI_CONCURRENT_CLIENT_GETAUTOIP_OK) 
		{
			TopBarSWF.showPrompt1("IP: "+WifiEngine.GetIPAddress());
			prompt1Ok = true;
		}
    }   
	else if(!prompt1Ok && WifiEngine.GetIndexofSelectAP() >= 0){
		TopBarSWF.showPrompt1("IP: "+WifiEngine.GetIPAddress());
		prompt1Ok = true;
...
function showSoftapInfo(){
	...
	if(EZCASTPRO_MODE==8075){
		...
		{
			setSoftapPosition(0);
			mySsid_24g=WifiEngine.getssid_Probox24g();
			trace("mySsid_24g"+mySsid_24g);
			TopBarSWF.ProboxshowshowSSID_24G("");
			TopBarSWF.showSSID(mySsid_24g);
		...
		else
		{
			...
			mySsid_24g=WifiEngine.getssid_Probox24g();
			trace("mySsid_24g"+mySsid_24g);
			TopBarSWF.ProboxshowshowSSID_24G(mySsid_24g);
		}
	...
	if(isEZCastpro() && SystemInfo.getPskHideStatus() != 0)
		passwdShowEnable = false;
	...
	currentMC.ssid_name.text = WifiEngine.getssid();
		MainSWF.ssid_txt = WifiEngine.getssid();
	showHostname();
	if(currentSWF == EZCAST_SWF)
		currentMC.bgMC.init();
}
...
function key_up_fun()
{
	trace("garen long_key "+long_ley_accept + key_midpress_accept);
	...
	else if(key_midpress_accept)
	{
		var channel_mode=WifiEngine.getChannelMode();
		trace("===============channel_mode========================"+channel_mode);
		if(channel_mode==WifiEngine.CHN_RTL_BAND_24G)
		{
			trace("===============switch to 5G=============");
		...
		else if(channel_mode==WifiEngine.CHN_RTL_BAND_5G)
		{
...
keyObject.onKeyDown = function() {
	...
	switch (whichKey) {
		...
		case CSwfKey.SWF_MSG_KEY_V:
			if(keyUpCheck >= 0)
			{
				trace("garen V clear "+keyUpCheck);
				clearInterval(keyUpCheck);
				keyUpCheck = -1;
			}
		...
		case CSwfKey.SWF_MSG_KEY_W:
			...
			if(curDongle == WifiEngine.WIFI_REALTEK_8821CU)
			{
				key_midpress_accept = false;
				if(timer_midcheck >= 0)
				{
				...
				timer_midcheck = setInterval(key_midpress_check, 2000);	
			}
		...
		case CSwfKey.SWF_MSG_KEY_S:
			...
			if(timer_shortcheck == 0) {
				timer_shortcheck = setInterval(key_shortpress_check, 500);	
			} else {
				clearInterval(timer_shortcheck);
				timer_shortcheck = setInterval(key_shortpress_check, 500);
			}
			
			if(timer_longcheck == 0) {
				timer_longcheck = setInterval(key_longpress_check, 5000);	
			} else {
				clearInterval(timer_longcheck);
				timer_longcheck = setInterval(key_longpress_check, 5000);
			}
