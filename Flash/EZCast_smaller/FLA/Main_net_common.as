...
function net_top_bar_setting(){
	...
	if(wifi_mode_get_from_case==WifiEngine.WIFI_CONCURRENT_CLIENT_GETAUTOIP_OK || wifi_mode_get_from_case == WifiEngine.WIFI_CONCURRENT_CLIENT_GETAUTOIP_ERR || wifistatus == WifiEngine.WIFI_AUTO_IP_SUCCESS || wifistatus == WifiEngine.WIFI_AUTO_IP_ERROR){//WifiEngine.WIFI_CONCURRENT_CLEINT_GETAUTOIP_OK){
		...
		if(internetAPstatus){
			...
			if(isEZCastpro())
			{
				if(SystemInfo.getConnectMode()!=0&&iswLanMiracodeShowing==false&&EZCAST_SWF == currentSWF&&currentMC.wifi_display_on)
				{
					var ip:String;
					trace("EZCASTPRO_MODE="+EZCASTPRO_MODE);
					if(EZCASTPRO_MODE==8075&&SystemInfo.get_24g_or_5g()) //Probox 1:2.4g wifi  0:5g wifi
						ip=SystemInfo.getIP(2);
					else
						ip=SystemInfo.getIP(0);
					if(ip != "" && ip!="error" && ip)
					{
	   			 ...
	   			 else if(iswLanMiracodeShowing||donglewLanIpShowing)
				{
					if(SystemInfo.getConnectMode()==0&&EZCAST_SWF == currentSWF)
					{
						currentMC.clean_miracode();
						iswLanMiracodeShowing=false;
						currentMC.clean_dongle_ip();
						donglewLanIpShowing=false;
	...
	else if("miracast_probox.swf" == currentSWF)
	{
