...
var ok_string,cancle_string,forget_string,wait_string,warn1_string, psk_string;
...
var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime(); //To solve the IE cache ,by cxf
	...
	{			
		...
		else
		{
			...
			if(scan_result_num>1 || scan_wifi_cnt>6)   //fixed 20170927
			{		
				if(wifi_start_scan_id>0)			
				{
					clearInterval(wifi_start_scan_id);
...
function Get_language()
{
	...
	$.get(cgiurl, {type:'get_wifilist_text'}, function(text)
	{	
	//alert(text);
	console.log("Get_language: "+text);
		if(text.length)
		{
			text_start=0;
			...
			text_string=text_string_global;	
			$('#refresh').text(Get_text_from_textstring(text_string));//add by bink
			
			text_string=text_string_global;	
			$('#heard').text(Get_text_from_textstring(text_string));//add by bink
			
			text_string=text_string_global;	
			$('#setup').text(Get_text_from_textstring(text_string));//add by bink
			
			text_string=text_string_global;	
			$('#scan_string').text(Get_text_from_textstring(text_string));//add by bink
			$('#waiting_string').text(wait_string+"...");//add by bink
			
			text_string=text_string_global;	
			psk_string = Get_text_from_textstring(text_string);//add by bink
