...
ezcast_file()
{
	...
	if [ $ezcastpro_flag -eq 8075 ]; then
		...
		if [ -f $TOPDIR/sdk/library/net/rtl8189ES_linux_v4.3.10.1_13373.20150129/8189es.ko ];then
			rm $TOPDIR/sdk/library/net/rtl8189ES_linux_v4.3.10.1_13373.20150129/8189es.ko
		fi
	fi
}
ezbox_conf_file()
{	
	MC_FILE=$TOPDIR/scripts/mconfig.h
	ezcastpro_flag=`cat $MC_FILE | grep -q MODULE_CONFIG_EZCASTPRO_MODE && cat mconfig.h | grep MODULE_CONFIG_EZCASTPRO_MODE | awk '{print $3}' || echo 0`
	echo "ezcastpro flag:" $ezcastpro_flag	
	if [ $ezcastpro_flag -eq 8075 ];then		
		CFG_FILE=AM_FS_EZPROBOX.cfg		
		echo "Config to PROBOX!!!"	 
	else		
...
if [ "$input_arg" = "ezcastpro" ];then
	...
	echo "SNMP for EZCastPro start!!!"
	cd ../sdk/library/net-snmp/src/;make ;cd -
	cd ../sdk/library/net-snmp/build/;sh do_build.sh ;sh do_release.sh;cd -
	pwd
	echo "SNMP for EZCastPro done!!!"
    ...
    if [ -f ../sdk/library/net ];then
        echo "8188eu driver for EZCastPro!!!"
        sh build_RealtekDriver.sh 8188eu
        echo "8192cu driver for EZCastPro!!!"
        sh build_RealtekDriver.sh 8192cu
        echo "8192du driver for EZCastPro!!!"
        sh build_RealtekDriver.sh 8192du
...
#	    cd $TOPDIR/scripts
     ...
     if [ -d ../sdk/library/net ];then
    	    MC_FILE=../scripts/mconfig.h
	    ezcastpro_flag=`cat $MC_FILE | grep -q MODULE_CONFIG_EZCASTPRO_MODE && cat mconfig.h | grep MODULE_CONFIG_EZCASTPRO_MODE | awk '{print $3}' || echo 0`
	    echo "8189es 8811au driver for EZCastProBox!!!" :$ezcastpro_flag
	    if [ $ezcastpro_flag -eq 8075 ]; then
	    	sh build_RealtekDriver.sh 8189es
	    	sh build_RealtekDriver.sh 8811au
	    	echo " build nand eeprom for EZProbox start!"
	    		if [ -f ../sdk/bin/1213/nand/brecf648.bin ];then
		    		echo "nand build aready!!!	"
				
			else	
				cd ../sdk/bootloader/nand/scripts/;sh am_sdk_config.sh AM_13;make;cd -
				pwd
			fi
			
		    	if [ -f ../sdk/bin/1213/nand/eeprom.bin ];then
		    		echo "eeprom build aready!!!	"
				
			else	
				cd ../sdk/bootloader/eeprom/scripts/;make clean;sh am_sdk_config.sh AM_13;make;cd -
				pwd
			fi
		
		echo "nand eeprom for EZProbox done!"	
	    fi
	    pwd
#	    cd $TOPDIR/scripts
	    echo "8189es 8811au driver for EZCastPro done!!!"		
    fi
   	
	echo "eeprom for EZProjector start!"
	if [ -f ../sdk/bootloader ];then
		cd ../sdk/bootloader/eeprom/scripts/;make clean;sh am_sdk_config.sh AM_13;make;cd -
		pwd
	fi
