...
#define STREAMING_NETWORK_DBG(fmt, arg...) //printf("STREAMING_NETWORK_DBG[%s-%d]:"fmt"\n",__func__,__LINE__,##arg)
...
check4proxies( URL_t *url ) {
	...
	STREAMING_NETWORK_DBG("url->protocol ======== %s\n",url->protocol);
...
http_send_request( URL_t *url, off_t pos ) {
	...
	STREAMING_NETWORK_DBG("url->port =================%d\n",url->port);
	...
	if (network_useragent){
		STREAMING_NETWORK_DBG("network_useragent =================%s\n",network_useragent);
		snprintf(str, sizeof(str), "User-Agent: %s", network_useragent);
	}
	else{
		STREAMING_NETWORK_DBG("mplayer_version =================%s\n",mplayer_version);
	    snprintf(str, sizeof(str), "User-Agent: %s", mplayer_version);
    }
		
	STREAMING_NETWORK_DBG("str =================%s\n",str);
	http_set_field(http_hdr, str);
	...
	if(http_hdr->buffer)
		STREAMING_NETWORK_DBG("http_hdr->buffer =================%s\n",http_hdr->buffer);
...
//#define CSH_DEBUG_HTTP
#ifdef CSH_DEBUG_HTTP
	fd_set rfds;
	int len = 0;
	int retval = -1;	
	static int timeout_value = 10;
	struct timeval tv_select;
	tv_select.tv_sec = timeout_value;
	tv_select.tv_usec = 0;
#endif
	...
	do {
		...
		if(len<BUFFER_SIZE) {
			...
			if(retval <= 0){
				...
				if(FD_ISSET(fd,&rfds)){
					retval = recv( fd, response, BUFFER_SIZE, 0 );

					if(retval <= 0){
						if(retval == 0){
							
							printf("[%s,%d] peer has been closed\n",__FILE__,__LINE__);
							return NULL;
						}
						else{
							printf("select error");
		...
		http_response_append( http_hdr, response, len );
#else	

		
__READ_RESPONSE_RETRY:

		i = recv( fd, response, BUFFER_SIZE, 0 );
		if( i<0 ) {
			err = errno;
			if(err==EINTR && cnt<1000){
				cnt++;
				goto __READ_RESPONSE_RETRY;
			}
			if(err == EAGAIN && cnt<1000){
				cnt++;
				goto __READ_RESPONSE_RETRY;
			}
...
#define TIMEOUT_SELECT_NOPSTREAM
#ifndef TIMEOUT_SELECT_NOPSTREAM
	if( len<size ) {
		int ret;
	...
	//printf("[%s,%d] recv total ==== %d KB\n",__FILE__,__LINE__,len/1024);
	...
	if(len<size) {
		...
		if(retval <= 0){
			...
			if(FD_ISSET(fd,&rfds)){
				retval = recv( fd, buffer+len, size-len, 0);
