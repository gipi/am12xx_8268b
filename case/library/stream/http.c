...
#define STREAMING_HTTP_DBG(fmt, arg...) printf("STREAMING_HTTP_DBG[%s-%d]:"fmt"\n",__func__,__LINE__,##arg)
static unsigned my_read(int fd, char *buffer, int len, streaming_ctrl_t *sc) {

#define TIMEOUT_SELECT_IN_MY_READ
#ifndef TIMEOUT_SELECT_IN_MY_READ
  ...
  return pos;
	...
	struct timeval tv_get_data;
	struct timeval tv_no_data;
	tv.tv_sec = 10;
	tv.tv_usec = 0;
    unsigned cp_len = sc->buffer_size - sc->buffer_pos;
    if (cp_len > len)
	...
	while (pos < len) {
		...
		if(retval < 0){
			perror("select error");
			printf("[%s,%d] retval ====== %d,pos ==== %ld\n",__FILE__,__LINE__,retval,pos);
			return pos;

		}
		else if(retval > 0){
			if(FD_ISSET(fd,&rfds)){
				retval = recv(fd, &buffer[pos], len - pos, 0);
			    if (retval < 0){
					...
					printf("[%s,%d] recv 0 Byte data \n",__FILE__,__LINE__);
					
					return pos;
				}
				else{
					pos += retval;
					gettimeofday(&tv_get_data,NULL);
				}
			}
		}
		else{
			gettimeofday(&tv_no_data,NULL);
			printf("[%s,%d] tv_get_data.tv_sec======= %ld !\n",__FUNCTION__,__LINE__,tv_get_data.tv_sec);
			
			printf("[%s,%d] tv_no_data.tv_sec======= %ld !\n",__FUNCTION__,__LINE__,tv_no_data.tv_sec);
			if(tv_no_data.tv_sec - tv_get_data.tv_sec > 10){
				printf("[%s,%d] Time out >10 s,should return !\n",__FUNCTION__,__LINE__);
			}
		}


	}
	STREAMING_HTTP_DBG("normal pos =========================%ld\n",pos);
	return pos;
#endif
}
...
static int nop_streaming_start( stream_t *stream ) {
	...
	if( fd<0 ) {
		...
		if( http_hdr->body_size>0 ) {
//printf("%s::%d::body_size(%ld) --> Is it a bug???\n", __FUNCTION__, __LINE__, (long)http_hdr->body_size);
//			if( streaming_bufferize( stream->streaming_ctrl, http_hdr->body, http_hdr->body_size )<0 ) {
//				http_free( http_hdr );
...
* For set pcm infomation
* Param sample_rate 	44100 //< record the sample rate of music 
* Param channels    	2     //< record the channels of music
* Param bits_per_sample 16
* Param media_type 		1(PCM)
*/
typedef struct{
...
static media_info_t media_para;

__attribute__ ((visibility("default"))) int http_get_media_info(void * get_media_info)
{
	media_info_t *tmp = (media_info_t *)get_media_info;
	tmp->sample_rate = media_para.sample_rate;
	tmp->channels = media_para.channels;
	tmp->bits_per_sample = media_para.bits_per_sample;
	tmp->media_type = media_para.media_type;
	return 0;
}
// End By VICTOR
...
static int http_streaming_start(stream_t *stream, int* file_format) {
	...
	char *tmp1 = NULL;
	char *tmp2 = NULL;
	char sample_rate[32];				/**< record the sample rate of music */
	char channels[32];					/**< record the channels of music */
	char bits_per_sample[32];					/**< record the channels of music */

	media_para.sample_rate = 0;
	media_para.channels = 0;
	media_para.bits_per_sample = 0;
	media_para.media_type = 0;
	#endif
	// End By VICTOR
	...
	do
	{
		...
		if( http_hdr!=NULL && http_hdr->status_code==200 ) {
		    ...
		    if (accept_ranges)
				seekable = strncmp(accept_ranges,"bytes",5)==0;
		                        ...
		                        strncmp(server, "JVC", 3) == 0)) {// HACK for JVC server for DLNA DMP IOP testing----Add by Victor 20121226
			...
			if (accept_ranges){
				...
				/** By SimonLee, we assume it to be seekable if no "Accept-Ranges" indication */
				seekable = 1;
		    }
		#endif
		...
		STREAMING_HTTP_DBG("http_hdr->status_code =========== %d\n",http_hdr->status_code);
		switch( http_hdr->status_code ) {
			case 200: // OK
				...
				content_type = http_get_field( http_hdr, "Content-Type" );// RFC 1590
				if( content_type!=NULL ) {
					...
					//Add By VICTOR for PCM  2013.01.10
					#if 1
					printf("http_streaming_start:Content-Type: [%s]\n", content_type );
					tmp1 = strstr(content_type,"audio/L16");// RFC 4856
					tmp2 = strstr(content_type,"audio/L8");// RFC 4856
					if (tmp1 != NULL || tmp2 != NULL){
						unsigned int count = 0;

						media_para.media_type = 1;// media_type = 1 for PCM
						if (tmp1 != NULL) media_para.bits_per_sample = 16;
						else if (tmp2 != NULL) media_para.bits_per_sample = 8;
						else media_para.bits_per_sample = 0;
						
						if ((tmp1 = strstr(content_type,"rate="))){// RFC 4856
							tmp2 = tmp1 + 5;
							count = 0;
							//printf("max_len = %u\n", max_len);
							while(*(tmp2+count) != '\0' && '0' <= *(tmp2+count) && '9' >= *(tmp2+count)) count++;
							if (count > 0){
								memcpy(sample_rate, tmp2, count);
								sample_rate[count] = '\0';
								//printf("http_streaming_start:sample_rate: [%s]\n", sample_rate);
								sscanf(sample_rate, "%u", &media_para.sample_rate);
								//printf("http_streaming_start:samplerate: [%u]\n", media_para.sample_rate);
							}
						}

						if ((tmp1 = strstr(content_type, "channels="))){// RFC 4856
						#if 0	//Only for test Allion DMS
							media_para.channels = 1;
						#else
							tmp2 = tmp1 + 9;
							count = 0;
							while(*(tmp2+count) != '\0' && '0' <= *(tmp2+count) && '9' >= *(tmp2+count))	count++;
							if (count > 0){
								memcpy(channels, tmp2, count);
								channels[count] = '\0';
								//printf("http_streaming_start:channels: [%s]\n", channels);
								sscanf(channels, "%u", &media_para.channels);
								//printf("http_streaming_start:channel: [%u]\n", media_para.channels);
							}
						#endif
						}
					}
...
static int fixup_open(stream_t *stream,int seekable) {
	...
	STREAMING_HTTP_DBG("stream->streaming_ctrl->url->protocol ================= %s\n",stream->streaming_ctrl->url->protocol);
	int is_ultravox = strcasecmp(stream->streaming_ctrl->url->protocol, "unsv") == 0;
	STREAMING_HTTP_DBG("is_ultravox ================= %d\n",is_ultravox);
...
static int open_s1(stream_t *stream,int mode, void* opts, int* file_format) {
	...
	STREAMING_HTTP_DBG("seekable ===================== %d\n",seekable);
	STREAMING_HTTP_DBG("*file_format ===================== %d\n",*file_format);
