...
#define STREAMING_DEBUG(fmt,arg...) //printf("STREAMING_DEBUG[%s-%d]:"fmt"\n",__func__,__LINE__,##arg)
...
int stream_read_internal(stream_t *s, void *buf, int len)
{
  ...
  //STREAMING_DEBUG("s->type ==== %d",s->type);
  switch(s->type){
...
*/
  ...
  if(len<=0){
    ...
    s->eof=1;
	printf("[%s %d]len<=0,should redo stream_read_internal",__FILE__,__LINE__);
...
int stream_fill_buffer(stream_t *s){
  ...
  if (len <= 0){
  	printf("function:%s,line:%d,len <= 0 return\n",__FUNCTION__,__LINE__,len);
    return 0;
  }
  s->buf_pos=0;
  s->buf_len=len;
