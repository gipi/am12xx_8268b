...
*/
ATTR_VISIBLE
DBPhotoManager *DBPhotoOpen(char *path)
{
	DBPhotoManager *handler;
	int rc;
	...
	handler = (DBPhotoManager *)malloc(sizeof(DBPhotoManager));
	if(handler == NULL){
		return NULL;
	}
	memset(handler,0,sizeof(DBPhotoManager));
...
static void DBPhotoFreeView(DBPhotoManager *handler)
{
	DBPhotoViewInfo *pPhoto;

	if(handler == NULL){
		return;
	}

	handler->photoView.photos.totalphotos= 0;

	pPhoto = &handler->photoView.photos;
	if(pPhoto->photos.sortIndex){
		free(pPhoto->photos.sortIndex);
		pPhoto->photos.sortIndex = NULL;
	}
...
*/
ATTR_VISIBLE
void DBPhotoClose(DBPhotoManager *handler)
{
	if(handler == NULL){
		return;
	}
...
*/
ATTR_VISIBLE
int DBPhotoGetViewInfo(DBPhotoManager *handler)
{
	sqlite3_stmt *stmt;
	char *sql;
	int rc;
	...
	DBPhotoFreeView(handler);

		/** sql for getting song number */
	sql = "SELECT COUNT(id) FROM photo_db";
	...
	if(rc != SQLITE_OK){
		...
		if(rc == SQLITE_ROW) {
			handler->photoView.photos.totalphotos = sqlite3_column_int(stmt, 0);
		}
...
*/
ATTR_VISIBLE
int DBPhotoDoViewSort(DBPhotoManager *handler,int sortType)
{
	sqlite3_stmt *stmt;
	DBphotoInfo *pInfo;
	int i;
	...
	pInfo = &handler->photoView.photos;


	if(pInfo->totalphotos<=0){
		return 1;
	}
	...
	pInfo->sortIndex = (int *)malloc(pInfo->totalphotos * sizeof(int));
	if(pInfo->sortIndex == NULL){
		return 1;
	}
	memset(pInfo->sortIndex,0,pInfo->totalphotos * sizeof(int));

	switch(sortType){
		case DBPHOTO_SORT_BY_NAME:
			sprintf(sql,"SELECT id,name FROM photo_db ORDER BY name;");
			break;

		case DBPHOTO_SORT_BY_FILESIZE:
			sprintf(sql,"SELECT id,file_size FROM photo_db ORDER BY file_size;");
			break;

		case DBPHOTO_SORT_BY_FILEEXTENSION:
			sprintf(sql,"SELECT id,file_extension FROM photo_db ORDER BY file_extension;");
			break;

		case DBPHOTO_SORT_BY_TIME:
			sprintf(sql,"SELECT id,modify_time FROM photo_db ORDER BY modify_time;");
			break;
...
*/
ATTR_VISIBLE
char * DBPhotoGetSortedViewItem(DBPhotoManager *handler,int index)
{
	char *result=NULL;
	DBphotoInfo *pInfo;
	int id;
	...
	sprintf(sql,"SELECT * FROM photo_db WHERE id=%d;",id);
...
typedef struct _photoDbTable PhotoDbTable;

struct _photoDbTable
{
	int id;
	char *dir;
	char *name;
...
static void __DbphotoFreeTable(PhotoDbTable *table)
{
	if(table == NULL){
		return;
	}
...
int photo_stopFlag=0;

ATTR_VISIBLE
int DbphotoCreateDatabase(char *filesDir,char *dbFullPath)
{
	...
	/** create table */
	sprintf(sql,"CREATE TABLE photo_db ( id integer primary key,directory text,name text,file_size integer,modify_time text,file_extension text );");
	...
	sprintf(sql,"INSERT INTO 'photo_db' VALUES(:id_a,:dir_a,:name_a,:file_size_a,:mtime_a,:ext_a);");
	...
	filelist.fsel = NULL;
	ui_filelist_init(&filelist, filesDir, "jpg bmp jpeg tif", 0, FLIST_MODE_BROWSE_FILE);
	total = ui_filelist_get_filetotal_num(&filelist);

	if(total < 1){
		fprintf(stderr,"No photo Filse when creating dababase\n");
		sqlite3_finalize(stmt);
		sqlite3_close(db);
		free(sql);
		remove(dbFullPath);
...
ATTR_VISIBLE
int DbPhotoDeleteDatabase(char *path)
{
	if(path == NULL){
		return -1;
	}
...
*/
ATTR_VISIBLE
int DbPhotoGetCreateStat()
{
	return createProcess;
}

ATTR_VISIBLE
int DbPhotoStopCreate()
{
	photo_stopFlag = 1;
