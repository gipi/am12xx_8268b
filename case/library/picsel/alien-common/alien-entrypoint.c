 ...
 *
 * $Id: alien-entrypoint.c,v 1.55.10.1.2.1 2014/01/31 14:48:12 denis Exp $
 */

#include "picsel-config-defines.h"
 ...
 * and is mandatory if there is more than one app in the build.
 */
                                        ...
                                        int                 allowAutoStart)
{
    ...
    onlyOneApp = (entryPoints[0].desc != NULL &&
    ...
    if ( argc >= 2 )
    {
        ...
        if ( entryPoint == NULL )
        {
            ...
            /* We didn't recognise the appname - if autostart is allowed, we
             * only have one argument and there is only app built in the
             * binary, then treat the arg as a filename. Otherwise, print the
             * list of valid apps and exit */
            if (allowAutoStart && argc == 2 && onlyOneApp)
            {
                entryPoint = &entryPoints[0];
                startupData->filename = argv[1];
            }
            else
            {
    ...
    else if (allowAutoStart && onlyOneApp)
    {
