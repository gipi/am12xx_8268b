 ...
 *
 * $Id: picsel-config-fileviewer.h,v 1.69 2012/11/30 08:39:36 hugh Exp $
 * @file
 */
 ...
 *
 * If it is set to @ref PicselConfigFV_PageFit_SignificantContent, the
 * document will be shown at its initial zoom level
 * (@ref PicselConfigFV_PageFit_SignificantContent). It will attempt to
 * ensure that the first piece of significant content is on screen. If no
 * significant content is found on the first page then the document will be
 * positioned with the top-left corner of the first page on screen.
 * Significant content is defined as visible content: some text or an image,
 * not white space. This is not an expensive operation, both in terms of time
 * and memory.
 *
...
typedef enum PicselConfigFV_PageFit_Values
{
    ...
    PicselConfigFV_PageFit_Page,
    /**< Do not use; internal use only. */
    PicselConfigFV_PageFit_Width,
    PicselConfigFV_PageFit_None,
    PicselConfigFV_PageFit_SignificantContent,
 ...
 * - 3 = reserved
 * - 4 = zoom, but reapply any document fit (page/width/height) relative
...
/**
 * The number of Tiles to use, if 'Tiling' mode is enabled in the build.
 *
 * @warning This is experimental, and likely to change and/or be removed.
 *
 * The size of a tile is currently a quarter of the screen.  This setting
 * specifies how many tiles are allocated when the application starts up.
 * The tiles are used to cache rendered document content.  When more tiles
 * are available, more of the document can be panned without fuzzy
 * redraw appearing.
 *
 * The default value is 16.  The value shouldn't be set to be less than 9
 * (and more than 9 is advisable in any case) because there typically won't
 * be enough tiles for the visible area.
 */
#define PicselConfigFV_tilingNumTiles       "Picsel_tilingNumTiles"
