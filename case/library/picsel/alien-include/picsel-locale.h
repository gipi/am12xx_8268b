 ...
 *
 * $Id: picsel-locale.h,v 1.25 2012/10/17 11:36:08 alexms Exp $
 * @file
 */
 ...
 * Sets a language
 *
 * This configures the language which is used by the application for its
 * user interface display.
 *
 * @param picselContext   Set by AlienEvent_setPicselContext()
 * @param language        The language in RFC3066 format
 *
 * The format is in the form "en-gb", which is
 * Language=English, Dialect=British,
 * i.e. an ISO 639-1 Languge code
 ...
 * A full list of supported languages follows. Please consult your
 * release notes/contract to determine which are available in your build.
 *    - @c ar-ar   Arabic
 *    - @c az-cyrl-az  Azeri, Cyrillic - Azerbaijan
 *    - @c az-latn-az  Azeri, Latin - Azerbaijan
 ...
 *    - @c zh-tw   Chinese - Taiwan
 *
 * If this is not configured, the language will
 * fall back to the locale setting configured by PicselLocale_set()
 *
 * This function should only be called from AlienConfig_ready().
 * Any change in the locale settings will not affect the application
 * unless it is restarted.
 *
 * @post The language string is copied by Picsel and can be released after this call.
 *
 * @retval 1 if success, @retval 0 if failure.
 * Failure means that a valid locale string was not provided.
 *
 */
int PicselLocale_setLanguage(Picsel_Context *picselContext,
                             const char     *language);
 ...
 *
 * @param picselContext   Set by AlienEvent_setPicselContext()
 * @param currencySymbol  The string to use for currency
 *
 * @post The currencySymbol string is copied by Picsel and should
 *       be released after this call.
 *
 * @retval 1 if success, @retval 0 if failure.
 * Failure means that a valid currencySymbol string was not provided.
 *
 */
int PicselLocale_setCurrencySymbol(Picsel_Context *picselContext,
