 ...
 *
 * @file
 * $Id: alien-screen.h,v 1.37 2012/05/14 12:14:23 neilk Exp $
 */
...
/**
 * @c AlienInformation_xxx values used to communicate with
 * the Alien application. This is only sent by Picsel Smart Office-based
 * products
 */
enum
{
    /** Informs the Alien application that the page number has
     *  been changed; @c eventData is
     *  @ref AlienInformation_PageChangeData */
    AlienInformation_PageChange = 0x20000
};

typedef enum Picsel_PageChangeSource
{
    /** Informs the Alien application that the page number has changed due
     *  to the document being scrolled */
    Picsel_PageChangeSource_Scroll = (1<<16),

    /** Informs the Alien application that the page number has changed due
     *  to the user selecting a slide or tab from the document panel */
    Picsel_PageChangeSource_DocumentPanel,

    /** Informs the Alien application that the page number has changed due
     *  to the user selecting a page from the page selector */
    Picsel_PageChangeSource_PageSelector
}
Picsel_PageChangeSource;

/**
 * Instructs the host that the page has been changed due to the reason code
 * within the data structure. Sent with an @ref AlienInformation_PageChange
 * event.
 *
 * Passed as @c eventData for the @ref
 * AlienInformation_PageChange event.
 *
 */
typedef struct AlienInformation_PageChangeData
{
    /** The reason for the page change */
    Picsel_PageChangeSource source;

    /** The page currently being displayed (the first page is number 1). */
    unsigned int page;
}
AlienInformation_PageChangeData;
...
/**
 * Force the target device to enable/disable stereoscopic 3d rendering.
 *
 * This function is called by the Picsel library when it's required
 * to switch stereoscopic 3d mode on or switch it off (2d mode).
 *
 *
 * @param[in] alienContext      See PicselApp_start().
 * @param[in] enable            Indicates whether the device needs
 *                              to enable/disable stereoscopic 3d mode.
 *                              Values:  0 - disable, 1 - enable.
 *
 * @product Must be implemented for platforms where devices equipped with
 *          the autostereoscopic (glasses-free) 3d screen are supported.
 */
void AlienScreen_setStereo(Alien_Context *alienContext, int enable);
