 ...
 *
 * @file
 * $Id: picsel-app.h,v 1.62.10.1 2014/05/27 15:16:45 denis Exp $
 */
 ...
 * commands like @ref PicselCmdPanDown. However some Picsel libraries
 * expect literal keys such as @ref PicselKeyDown, and will call
...
typedef enum PicselInputType
{
     ...
     * @product Various Picsel products and applications.
     */
    PicselInputType_Key
}
PicselInputType;
...
enum
{
     ...
     * to be panned then this may be ignored.
     *
     ...
     *
     * If the Picsel library is not in a state where a document is available
     * to be zoomed, for example if no document has been loaded, then this
     * may be ignored.
     *
     ...
     * For Excel, move the currently highlighted cell to the first column
     * in this row.
     */
    PicselCmdLineStart,
    ...
    /**
     * Modifies the current selection by moving its unanchored point up by
     * one line.
     */
    PicselCmdSelectionLineUp,

    /**
     * Modifies the current selection by moving its unanchored point down by
     * one line.
     */
    PicselCmdSelectionLineDown,

    /**
     * Modifies the current selection by moving its unanchored point back by
     * one character.
     */
    PicselCmdSelectionCharBack,

    /**
     * Modifies the current selection by moving its unanchored point forward
     * by one character.
     */
    PicselCmdSelectionCharForward,

    /**
     * Modifies the current selection by moving its unanchored point to the
     * start of the current line.
     */
    PicselCmdSelectionLineStart,

    /**
     * Modifies the current selection by moving its unanchored point to the
     * end of the current line.
     */
    PicselCmdSelectionLineEnd,

    /**
     * Modifies the current selection by moving its unanchored point to the
     * start of the document.
     */
    PicselCmdSelectionDocStart,

    /**
     * Modifies the current selection by moving its unanchored point to the
     * end of the document.
     */
    PicselCmdSelectionDocEnd,

    /**
     * The indicator of the last position of PicselCmd_.
     * To avoid overlap issue, it should be smaller than the value of the
     * first PicselKey_, i.e. PicselKeyUp (66000)
     */
    PicselCmdLast,
...
enum
{
    ...
    PicselKeyBackspace=66012,  /**< The Backspace key */
    PicselKeyPageUp,     /**< The PageUp key */
    PicselKeyPageDown,   /**< The PageDown key */
    PicselKeyHome,       /**< The PageHome key */
    PicselKeyEnd,        /**< The PageEnd key */
    ...
    PicselKeySearch,     /**< A dedicated Search key, if available */
    PicselKeyDelete,     /**< The Delete or Del key */
...
enum
{
    ...
    /**
     * Prime the app to load the specified file, but don't actually load it
     * yet until the current document is closed.
     *
     * @warning For internal use only.
     */
    PicselApp_LoadFlags_Defer            = 0x40,
    /**
     * Handle the file as if it had been passed into PicselApp_start as
     * the document to load at startup.
     *
     * @product UE2 Based Products.  This may be expanded to other applications.
     */
    PicselApp_LoadFlags_StartUpFile      = 0x80
};
...
/**
 * Inform the Picsel library of the current "best guess" from the platform IME
 * for text input by the user.  If this event is handled by the Picsel app,
 * this text may be temporarily displayed until PicselApp_commitText() is
 * called to give the final version of the user input text.
 *
 * @param[in] picselContext Set by AlienEvent_setPicselContext().
 * @param[in] composingText Current "best guess" of composing text (UTF-8)
 *
 * @return  The queue status, normally 1. See @ref TgvAsync_Queue.
 *
 * @product UE2 Based Products
 *
 * @ingroup TgvUserInput
 *
 */
int PicselApp_composingText(Picsel_Context *picselContext,
                            const char     *composingText);

/**
 * Inform the Picsel library that a user input text string composed using the
 * platform IME has been finalised by the user and should be inserted into the
 * document, if that is appropriate.  A call to this function may be preceded
 * by 1 or more calls to PicselApp_composingText().
 *
 * To delete multiple characters preceding the current selection/caret, set
 * "finalText" as a string of backspace characters (0x08) of the length of
 * the number of characters to delete.  Do not mix backspace characters with
 * printable characters in the "finalText" string.
 *
 * @param[in] picselContext Set by AlienEvent_setPicselContext().
 * @param[in] finalText     Finalised text (UTF-8)
 *
 * @return  The queue status, normally 1. See @ref TgvAsync_Queue.
 *
 * @product UE2 Based Products
 ...
 *
 */
int PicselApp_commitText(Picsel_Context *picselContext,
                         const char     *finalText);

/**
 * Inform the Picsel library that a keyboard has been exposed or hidden.
 *
 * @param[in] picselContext  Set by AlienEvent_setPicselContext().
 * @param[in] softKeyboard   1 => on-screen soft keyboard.
 *                           0 => hardware keyboard.
 * @param[in] visible        1 => The keyboard is visible.
 *                           0 => The keyboard has been hidden.
 *
 * @return  The queue status, normally 1. See @ref TgvAsync_Queue.
 *
 * @product UE2 Based Products
 ...
 *
 */
int PicselApp_keyboardVisible(Picsel_Context *picselContext,
                              char            softKeyboard,
                              char            visible);
...
/**
 * Call this function to inform the Picsel library that the application has
 * lost the operating system focus.
 *
 * A corresponding call to PicselApp_activate should be made if and when
 * the application regains the focus.
 *
 * Note that this function is not currently required to be called for
 * FVDK-based products.  Please check with a Picsel representative as to
 * whether this function is required for other products.
 *
 * Note further, that currently for products which do require calls to
 * this function, then PicselApp_suspend and PicselApp_resume should not be
 * used.
 *
 * @param[in] picselContext Set by AlienEvent_setPicselContext().
 *
 ...
 * @retval 0  On failure.
 */
int PicselApp_deactivate(Picsel_Context *picselContext);

/**
 * Call this function to inform the Picsel library that the application has
 * gained the operating system focus.
 *
 * Note that this function is not currently required to be called for
 * FVDK-based products.  Please check with a Picsel representative as to
 * whether this function is required for other products.
 ...
 * @retval 0  On failure.
 */
int PicselApp_activate(Picsel_Context *picselContext);
