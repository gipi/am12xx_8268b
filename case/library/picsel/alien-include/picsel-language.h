 ...
 *
 * $Id: picsel-language.h,v 1.12 2011/02/21 09:25:05 yongchul Exp $
 * @file
 */
 ...
 *
 * If Picsel TGV library is built for using external fonts only, then
 * no fonts are built into the library, so external fonts should be used
 * instead from local file system: @see PicselConfig_fontsDirectory, and
 * it still needs to call Picsel_Font_Register_xxxx function even though
 * external fonts will be used.
 *
 * @{
 */
