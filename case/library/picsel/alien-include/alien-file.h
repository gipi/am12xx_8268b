 ...
 *
 * @file
 * $Id: alien-file.h,v 1.51 2010/08/04 14:18:46 mary Exp $
 */
 ...
 *  - AlienFile_getMimeTypeFromFileDescriptor (no return value)
 ...
 *
 * Get the MIME type, also known as the Internet Media type, for
 * the specified file.  Additional parameters, such as charset, are
 * excluded.
 *
 * This function will be called when the Picsel library encounters a file with
 * no file extension.
 *
 * Any memory allocated for the mimeType must not be deallocated until
 * application shutdown.
 ...
 *                            or memory could not be allocated for the string.
 */
void AlienFile_getMimeTypeFromFileDescriptor(Alien_Context       *alienContext,
                                             int                  fd,
                                             const char         **mimeType);
