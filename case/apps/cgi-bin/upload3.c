...
static int  Ck_CAfileExist(){
	int ca_exist= access("/etc/cert/CA.pem",F_OK);
	int cert_exist= access("/etc/cert/Certificate.pem",F_OK);
	int psk_exist=access("/etc/cert/Privatekey.pem",F_OK);
	//char cmd_rmpem[32]="rm /tmp/*.pem";
	//htmlsetting_func("system_cmd", (void*)cmd_rmpem, strlen(cmd_rmpem), NULL, NULL);
	ca_exist=ca_exist+cert_exist+psk_exist;
	return ca_exist;
}
...
int cgiMain(int argc, char *argv[])
{
	...
	if(!isSubtitle(rfile)){
		...
		if(Is_CApemfile(rfile)){
			...
			snprintf(lfile, sizeof(lfile), "/etc/cert/Privatekey.pem"); 
		}
