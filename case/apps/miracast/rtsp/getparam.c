...
long SendGetParameters(CRTSPParser* p, char* szParams)
{
	...
	if (p->m_bLegacy && !p->m_bServer && p->m_bRtspConnected) {
		// LEGACY: stack does not support receiving GET_PARAMETER from snk
...
long SendGetParametersResponse(CRTSPParser* p, int nCSeq, char* szGetParamsResp)
{
	return SendResponse(p, nCSeq, 200, "OK", "", szGetParamsResp);
}
