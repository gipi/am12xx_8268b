...
{
	if (p->m_bServer)
		return WFD_SendTeardownTrigger(p);
	else
		return WFD_SendTeardown(p);
}
...
long WFD_SendTeardownTrigger(CRTSPParser* p)
{
	...
	int nLen = wfd_snprintf_(szBody, RTSP_SEND_BUFFER, "wfd_trigger_method: TEARDOWN\r\n");
...
long WFD_SendTeardown(CRTSPParser* p)
{
	...
	// sink transition to M9
	p->m_eState = WFD_RTSP_M09;
	p->m_ulStartTime[p->m_eState] = p->m_pGetTick(p->m_pContext);
...
long WFD_SendTeardownResponse(CRTSPParser* p, int nCSeq)
{
	...
	//WFD_RTSP_LOG(p, RTSP_LOG_NOTICE, "%s - To Send = %d", __FUNCTION__, WFD_QueuedBufferCount(&p->m_bqToSend));
	//WFD_RTSP_LOG(p, RTSP_LOG_NOTICE, "%s - Sent = %d", __FUNCTION__, WFD_QueuedBufferCount(&p->m_bqSent));
