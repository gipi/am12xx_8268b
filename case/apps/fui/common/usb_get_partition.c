...
get_bdev_partnums(const char *disk_device)
{
	...
	//printf("%s %d==============disk_device:%s\n",__FILE__,__LINE__,disk_device);
...
int get_card_mbrinfo(const char *disk_device)
{
	int fd;
	int result;
	int parts = 0;
	...
	for (slot = 1; slot <= 4; slot++,p++) {
		if ((p->boot_ind != 0) && (p->boot_ind != 0x80)) {
			...
			* without a partition table.
			*/
			fb = (struct fat_boot_sector *)mbr_buffer;
			if (slot == 1 && fb->reserved && fb->fats && fat_valid_media(fb->media)) {
				dbg("no mbr but a fat boot sector\n");
				mbrValid =0x00;
			} else {
