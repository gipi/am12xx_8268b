...
#define JPEGDEC_YCbCr440_SEMIPLANAR 0x010002U
#define JPEGDEC_YCbCr444_SEMIPLANAR 0x040001U
...
typedef struct JpegDecInitParam
{
	void *pHeap;
	u32 reg_rw_trace;
	u32 mem_usage_trace;
...
typedef struct
{
	...
	unsigned long reg12;
	unsigned long decoded_stream_size;
	void *file_handle;
	long (*read)(void *, unsigned char *, unsigned long);
	long (*seek_set)(void *, unsigned long);
...
/* Initialization */
JpegDecRet JpegDecInit(JpegDecInst *pDecInst, JpegDecInitParam_t *param);
...
void *JpegDecGetDWL(JpegDecInst decInst);

void JpegDecReset(JpegDecInst decInst);
