...
#ifndef __PHOTO_MIDWARE_H__
#define __PHOTO_MIDWARE_H__
...
/* 图片中间件IO层 */
typedef struct tagImgIOLayer
{
	void *(*img_fopen)(char *, char *);
	int (*img_fclose)(void *);
	long (*img_fread)(void *, unsigned char *, unsigned long);
	long (*img_fseek_set)(void *, long);
	long (*img_fseek_cur)(void *, long);
	long (*img_fseek_end)(void *, long);
	long (*img_ftell)(void *);
...
{
	int result;                    //发出当前命令后得到的返回值-1 decoded error, 0 OK, 1 no support, 2 be stopped
...
/* 打开图片解码器API */
void *image_dec_open(IMG_IO_LAYER *img_io);
