...
static char is_calibrate_clkdly_done=0;
...
**/
int pll_change_pll(unsigned int clock)
{
	struct am_chpll_arg chpll_arg;
	struct stat stbuf;
	int fd=-1, fd_pmu=-1;
	...
	printf("PLL CLOCK==%d\n",clock);
	#if 0
	if(is_calibrate_clkdly_done==0){
		fprintf(stderr, "call pll_calibrate: %s\n", filename);
		return 0;
	}
	#endif
	if( stat(filename, &stbuf) != 0 ) {
	...
	ioctl(fd_pmu, AM_PMU_PLOW, &chpll_arg); //AM_PMU_PLOW and the AM_PMU_PHIGH had the same effect
	rtn =0;

	printf("%s,%d:Change PLL Succeed ==%d\n",__FILE__,__LINE__,clock);
CHANGE_PLL_END:
	if(fd>=0)
		close(fd);
	if(fd_pmu!=-1)
