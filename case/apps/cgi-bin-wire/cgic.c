...
void cgiHeaderCookieSetStringforPro(char *name, char *value, int secondsToLive,
	char *path, char *domain)
{
	/* cgic 2.02: simpler and more widely compatible implementation.
		Thanks to Chunfu Lai. 
...
void cgiHeaderCacheControl(char *str) {
    fprintf(cgiOut, "Cache-Control: %s\r\n", str);
}
void cgiHeaderPragma(char *str) {
    fprintf(cgiOut, "Pragma: %s\r\n", str);
}
void cgiHeaderExpires(unsigned int number) {
    fprintf(cgiOut, "Expires: %d\r\n", number);
}
