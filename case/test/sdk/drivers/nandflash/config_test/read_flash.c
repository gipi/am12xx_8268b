...
unsigned int static rd(unsigned char * sdfilename,unsigned char * nffilename,unsigned char * buf,unsigned int w_sum)
{
	...
	for (loop2=0;loop2<w_sum;loop2++)
	{
		...
		{
			printf("<Test Info> read file : %s from sdcard error  at: %d times!\n",filename2,loop2+1);
			return 0x00;
		}
		close(fp2);
		fp2=NULL;
...
//return 0:0k    1:fail
int Read_flash(unsigned int sum_param,char *param_buf)
{
	unsigned char *TmpBuf=(void *)malloc(16*1024);
	int param[10];
	char t_buf[10];
	...
	printf("<Test Info> read sum == %d\n",param[2]);
	for (t_i=0;t_i<param[1];t_i++)
	{
		mk_filename(mediafilename,t_i);
		mk_filename(nandfilename,t_i);
		if (0==rd(mediafilename,nandfilename,TmpBuf,param[2]))
		{
			printf("<Test Info> read flash error!!\n");
			return 1;
		}
	}
