...
int main(int argc, char*argv[])
{
	...
	rtp_session_set_payload_type(session,33);
	...
	while(cond)
	{
		have_more=1;
		while (have_more){
			err=rtp_session_recv_with_ts(session,buffer,64*1024,ts,&have_more);
		...
		ts+=3600;
		ortp_message("--Receiving packet:%d\n",total_recv_size);

		if(total_recv_size >= 200*1024){
			break;
		}
	}

	ortp_message("---------- ortp out\n");
	if(outfile){
