...
//GB2312码转为UTF-8码   
  ...
  void test_gb2312_to_utf8()
  {
  	char *pstr = "中国";
		char outBuf[BUF_SIZE]={0,};
		size_t inlen = strlen(pstr);
		size_t outlen = BUF_SIZE;
		int i = 0;
		
		printf("\n\n------------gb2312_to_utf8 begin------------\n");		
		printf("gb2312: ");
		for(i = 0; i < strlen(pstr); i++)
			printf("0x%x, ", (unsigned char)pstr[i]);
		printf("\nutf8: ");
		if(gb2312_to_utf8(pstr, &inlen, outBuf, &outlen) != 0)
			printf("----fail!\n");
		for(i = 0; i < BUF_SIZE-outlen; i++)
			printf("0x%x, ", (unsigned char)outBuf[i]);
		printf("\n------------gb2312_to_utf8 end------------\n");		
  }
  
  void test_gb2312_to_utf16le()
  {
  	char *pstr = "中国";
		char outBuf[BUF_SIZE]={0,};
		size_t inlen = strlen(pstr);
		...
		printf("\n\n------------test_gb2312_to_utf16le begin------------\n");		
		printf("gb2312: ");
		for(i = 0; i < strlen(pstr); i++)
			printf("0x%x, ", (unsigned char)pstr[i]);
		
		printf("\nutf16le: ");
		if(gb2312_to_utf16le(pstr, &inlen, outBuf, &outlen) != 0)
			printf("----fail!\n");
		for(i = 0; i < BUF_SIZE-outlen; i++)
			printf("0x%x, ", (unsigned char)outBuf[i]);
		printf("\n------------test_gb2312_to_utf16le end------------\n");		
  }   
