...
#if 1
void * medialib_malloc(unsigned int size)
{
	return malloc(size);
}

void medialib_free(void *p)
{
	if(p)
		free(p);
}
...
static int _init_db_info(char *rootpath,char*file_format,char*db_name,int attrmask,int mtype){
	...
	if(album_info.ml_info==NULL){
		album_info.ml_info = medialib_malloc(sizeof(struct medialib_info));
		if(album_info.ml_info==NULL){
	...
	album_info.ml_info->rootpath = medialib_malloc(str_len+1);
	...
	album_info.ml_info->file_format= medialib_malloc(str_len+1);
...
static int _free_db_info(int need_free_name)
{
	if(album_info.ml_info){
		if(album_info.ml_info->rootpath)
			medialib_free(album_info.ml_info->rootpath);
		if(album_info.ml_info->file_format)
			medialib_free(album_info.ml_info->file_format);
		_db_free_medialib_info_name(album_info.ml_info,need_free_name);
		medialib_free(album_info.ml_info);
...
{	
	if(cache_info->viewhandle==NULL){
		...
		cache_info->idxbuf=(unsigned int*)medialib_malloc(sizeof(unsigned int)*cache_info->idxbuf_len);
...
static int _db_cache_release(db_cache_t *cache_info)
{
	if(cache_info->viewhandle){
		if(cache_info->idxbuf){
			medialib_free(cache_info->idxbuf);
		...
		if(cache_info->value_type==TYPE_VALUE_STRING){
			if(cache_info->value!=NULL){
				medialib_free(cache_info->value);
			}
			cache_info->value_type = TYPE_VALUE_UNKNOW;
			cache_info->value = NULL;
			cache_info->value_len = 0;
...
static int _db_realloc_idxbuf(db_cache_t *cache_info)
{
	...
	tmpbuf = medialib_malloc(tmpbuf_len);
	if(tmpbuf==NULL){
								...
								int valuetype,modify_mode_t mode,db_cache_t *cache_info,int action_type)
{
	...
	if(action_type==ACTION_MODIFY){
		...
		if(cache_info->value_type==TYPE_VALUE_STRING){
			if(cache_info->value==NULL){
				cache_info->value = medialib_malloc(value_len+1);
				if(cache_info->value==NULL){
					cache_info->value_len = 0;
					rtn = -1;
					goto ADD_ITEM_END;
...
{
	struct sdram_dbname_s * sdram_dbname=(struct sdram_dbname_s *)medialib_malloc(sizeof(struct sdram_dbname_s));
	if(sdram_dbname){
		sdram_dbname->start_addr = (unsigned int)medialib_malloc(SDRAM_DB_SIZE);
		...
		if(sdram_dbname->start_addr==0){
			medialib_free((void*)sdram_dbname);
			sdram_dbname = NULL;
		}
...
static void __free_sdram_db_name(struct sdram_dbname_s * sdram_dbname)
{
	if(sdram_dbname){
		...
		if(sdram_dbname->start_addr){
			medialib_free((void*)sdram_dbname->start_addr);
			sdram_dbname->start_addr = 0;
			sdram_dbname->mem_size = 0;
		}
		medialib_free(sdram_dbname);
	}
}
...
static int __fill_sdram_db_info(char* dbname,int index)
{
	...
	if(index<SDRAM_DB_NUM){
		...
		sdram_db_info[index].swf_db_name= medialib_malloc(str_len+1);
		if(sdram_db_info[index].swf_db_name){
			...
			if(sdram_db_info[index].sdram_db_name==NULL){
				medialib_free(sdram_db_info[index].swf_db_name);
...
int _db_fill_medialib_info_name(char *db_name,struct medialib_info *medialib_info)
{
	...
	if(db_name && medialib_info){
		if(medialib_info->s_type == VRAM_STORAGE){
			...
			medialib_info->name= medialib_malloc(str_len+1);
...
void _db_free_medialib_info_name(struct medialib_info *medialib_info,int need_free_name)
{
	...
	if(medialib_info->s_type == VRAM_STORAGE){
		if(medialib_info->name){
			medialib_free(medialib_info->name);
			medialib_info->name = NULL;
		}
	}
	else if(medialib_info->s_type == SDRAM_STORAGE){
...
static int medialib_createdb(char *rootpath,int attrmask,char* fileformat,char *dbname,enum minfo_type  m_type){
	int ret=-1;
	db_info("");
...
static int medialib_destorydb(int m_type,char* db_name)
{
	int s_type=0;
	void *db_realname=NULL;
	int rtn=0;
...
static int  medialib_updatedb(int dbhandle){
	int rtn = 0;
...
static int medialib_get_thread_status()
{
	return album_info.thread_status;
}


static int medialib_open_views(int dbhandle,int metadata_as,char* value,int view_mode,int viewbasedon)
{
	...
	if(view_mode==VIEW_MODE_PURE_SORT)
		...
		if(ret==0){
			...
			if(_db_change_buf_format(value,&strsrc_len,1,info_buffer,&desbuf_len)==-1){
...
OPEN_VIEWS_END:	
	...
	return ((int)viewhandle);
}



static int medialib_close_views(int viewhandle){
	int rtn = 0;
	if(viewhandle!=0)
...
static int medialib_get_num(int viewhandle){
...
static int medialib_del_items(int viewhandle, unsigned int idx){
	int rtn=0;
...
static int medialib_add_items(int dbhandle,char* filename,int tagvalue)
{
...
static int medialib_find_item(int viewhandle,int metadata_as, char* value,int findmode){
	int rtn;
	int indxfind=-1;
	unsigned int valuereal=0;
	int metadata;
...
static int medialib_modify_items(int viewhandle,int idx,int metadata_as,char *value,int modify_mode){
	int metadata = 0;
	int ret=0;
	unsigned int valuereal=0;
	int rtn=0;
...
static char* medialib_get_info(int viewhandle,int index,int info_mode)
{
	#if 0
...
{
	int create_status=0;
	char *rootpath = "/mnt/card";
	int attrmark=ML_ATTR_FILE_NAME|ML_ATTR_FILE_SIZE|ML_ATTR_FILE_TIME|ML_ATTR_FILE_PATH|ML_ATTR_EXIF_TIME|ML_ATTR_USER_TAG1;
	char fileformat[]="jpg bmp jpeg";
	char dbname[]="sdcard_db";
	enum minfo_type  minfo=MINFO_TYPE_PHOTO;
	db_info("Create DB!!!!!!!!!!!!!!!!!");
	medialib_createdb(rootpath,attrmark,fileformat,dbname,minfo);
	while(1){
		create_status =medialib_get_thread_status();
		if(create_status!=THREAD_DB_CREATEING)
			break;
		sleep(1);
	}
	db_info("DB Create END status=%d",create_status);
}
